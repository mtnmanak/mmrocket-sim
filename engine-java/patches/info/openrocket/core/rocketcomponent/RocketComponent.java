...
public abstract class RocketComponent implements ChangeSource, Cloneable, Iterable<RocketComponent> {
	...
	/**
	 * PERF PATCH: memo for {@link #getComponentLocations()}, the hottest method
	 * in a flight. Null means "not computed"; cleared for every component on
...
//	}
	...
	public RocketComponent clone() throws CloneNotSupportedException {
		...
		// PERF PATCH: super.clone() is a shallow field copy, so the clone would
	...
	public Coordinate[] getComponentLocations() {
		// PERF PATCH (memoize): this is the single hottest method in a flight —
		// ~45 % inclusive (recursion-safe measure) in a CPU profile of a LEM-IV
		// flight: 2,336 RK4 steps, 634 stored samples. It is a
		// recursive walk to the rocket root allocating a Transformation
		// (double[3][3]) and a Coordinate array per level, and the automatic
		// ring-radius accessors (CenteringRing.getInnerRadius,
		// RadiusRingComponent.getOuterRadius) call it via toRelative twice per
		// accessor, per ring, per RK4 sub-step, for the whole flight — on
		// geometry that cannot change while a simulation runs.
		// Invalidated from componentChanged(), the hook upstream documents for
		// exactly this ("subclasses may override this method to e.g. invalidate
		// cached data"), which every component receives on every
	...
	public final Coordinate[] toRelative(Coordinate c, RocketComponent dest) {
		...
		// PERF PATCH (loop-invariant hoist): upstream evaluates
		// `this.getComponentLocations()[0].add(c)` INSIDE the loop, once per
		// destination location, though it does not depend on coordIndex. On a
		// single-instance design destLocs.length is 1 and this saves nothing;
		// where `dest` is a clustered or multi-instance component it removes a
		// full recursive walk per extra instance.
		// getComponentLocations() is a recursive walk to the rocket root that
		// allocates a Transformation (double[3][3]) per level, and toRelative is
		// the hot path of the automatic ring-radius accessors. Hoisting is
		// arithmetically identical — same operands, same order — so every result
		// is bit-for-bit unchanged.
		final Coordinate thisLoc = this.getComponentLocations()[0].add(c);
		Coordinate[] toReturn = new Coordinate[destLocs.length];
		for (int coordIndex = 0; coordIndex < destLocs.length; coordIndex++) {
			toReturn[coordIndex] = thisLoc.sub(destLocs[coordIndex]);
		}
