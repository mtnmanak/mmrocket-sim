...
public class FreeformFinSet extends FinSet {
	...
	public void setPoints( ArrayList<Coordinate> newPoints, boolean validateFinTab) {
		...
		if (outlineRefused = intersects()) { // MMRocket Sim patch: see isOutlineRefused()
	...
	private boolean intersects(final int targetIndex) {
		...
		for (int comparisonIndex = targetIndex+1; comparisonIndex < (points.size() - 1); ++comparisonIndex) {
			...
			if (targetLine.intersectsLine(comparisonLine)) {
				...
				log.warn(String.format("                   between (%s, %s) => (%s, %s)", Double.toString(pt1.x), Double.toString(pt1.y), Double.toString(pt2.x), Double.toString(pt2.y))); // PATCH(teavm-format-g), see below
				log.warn(String.format("                       and (%s, %s) => (%s, %s)", Double.toString(pc1.x), Double.toString(pc1.y), Double.toString(pc2.x), Double.toString(pc2.y))); // PATCH(teavm-format-g), see below
				return true;
			}
		}
		return false;
	...
	// ===== MMRocket Sim patch (audit 2026-09-22). Everything this patch adds sits
	// here, below upstream's last line, and the lines it changes are changed in
	// place, so every line number above still matches upstream 24.12 - the app's
	// comments cite them (tree/finOutline.ts, position.ts, solidMesh.ts, ...).
	// See patches/LEDGER.md, rocketcomponent/FreeformFinSet.java.
	//
	// PATCH(teavm-format-g): the two "between/and" log lines in intersects(int)
	// were "%g" and are now "%s" with Double.toString, exactly as in
	// BasicEventSimulationEngine. TeaVM's Formatter lacks the %g conversion, so
	// every refused outline threw "Unknown format conversion: g" out of the log
	// line itself and took the whole build down, while the JVM logged it and
	// rolled back. Log-only.

	/** Set by setPoints(ArrayList, boolean); see isOutlineRefused(). */
	private boolean outlineRefused = false;

	/**
	 * Whether the most recent setPoints(...) call was REFUSED: its outline
	 * crossed or touched itself (a repeated point does it - the zero-length edge
	 * touches its neighbours' neighbours), so upstream rolled the points back to
	 * the previous outline, which on a freshly built fin set is the
	 * constructor's default.
	 * <p>
	 * Upstream reports that refusal only as a log line, so on the desktop a
	 * refused outline flies the DEFAULT fin without a word. The JS bridge
	 * (api.ComponentFactory) reads this flag and throws a message naming the fin
	 * set instead, so a design never flies a fin other than the one it draws.
	 *
	 * @return true if the last setPoints call rolled its outline back
	 */
