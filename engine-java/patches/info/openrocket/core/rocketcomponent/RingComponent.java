...
public abstract class RingComponent extends StructuralComponent implements BoxBounded, Coaxial {
	...
	public double getRotationalUnitInertia() {
		// MMRocket Sim patch (audit 2026-09-22, code review E1): the ring's own
		// axial term PLUS its instances' parallel-axis spread. See
		// instanceSpreadUnitInertia() and patches/LEDGER.md, "Correctness fixes".
		final double own = ringRotationalUnitInertia(getOuterRadius(), getInnerRadius());
		final double spread = instanceSpreadUnitInertia();
		// Structural, not arithmetic: an on-axis ring (every centering ring,
		// bulkhead, coupler, engine block and centreline inner tube) returns
		// `own` untouched, so no floating-point operation is added to it.
