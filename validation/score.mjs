 ...
 * app actually launches with. It was missing until 2026-08-26: the Kbf score
 * was being carried by HAND from sitting to sitting (working-notes: "Kbf
 * 17/175 carried forward — score.mjs has no Kbf flag"), which meant the only
 * model most users ever fly was the one model no change could be scored
 * against. A kernel edit gated on `rogersKbf` was unverifiable by this harness
 * on the exact model it shipped to.
 *
...
let gatePass = 0;
const out = [];
out.push(`# Anchor scorecard — ${modelName}`);
out.push('');
