...
sitting in that table looking exactly like a fresh one is worse than an honest blank. A flight
flown with **auto (optimal)** ticked counts only when its rounded optimum is the delay the file names
for that motor — the file records one delay per motor, and a flight at another would be filed under
the wrong one — so otherwise that flight is left out, and the configuration saves as *not simulated*
unless it has an earlier, still-matching flight at the named delay, which is written instead. To
carry the auto flight, type the delay it recommends (typing a delay unticks auto) and **Launch**
again. Only the
...
**If the app stops on an error.** An error the app cannot draw past replaces the page with a recovery panel. **⬇ Download the autosaved design** saves your design as an `.ork`, or, when the design cannot be written as one, the autosave exactly as the browser holds it (a `.json` file to attach to a bug report). **Start fresh…** asks first, then deletes only the autosaved design and the remembered workspace tab and reloads; saved runs, imported motors, custom presets and preferences are kept. If the autosave holds another open tab's design by then, starting fresh leaves it alone and reopens with it. When only one panel fails — the flight report, the plots, the drag chart or the run history — that panel says it could not be drawn and offers **↻ Try again**, and the rest of the app keeps working.
