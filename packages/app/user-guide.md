...
Two things the database cannot tell you, because no published thrust-curve file carries them: **the pressure the curve was measured at** (every curve is treated as a sea-level measurement) and **the motor's nozzle exit diameter**. That second one is yours to type, under the stage, and it is what lets the app add the thrust a motor gains as the air thins — see *Multi-Stage and Clustered Rockets → Nozzle exit diameter*. Take it from the motor's own data sheet or measure the hardware; all the app can check is that it is not wider than the casings it comes out of.
...
Before you fly, the Launch Conditions panel (in the **Motors & Launch** workspace) sets the pad and the weather. It collects nine fields: **launch rod/rail length** (default 1 m), **rod angle** (0°, range −30…30°), **average wind**, **wind gust sigma** (standard deviation of gusts about the average), **site altitude** (0–10,000 m), **latitude** (default 28.61°, roughly Florida), **temperature** (blank = ISA standard), **station pressure** (blank = ISA standard — but read the next paragraph, because *which* standard depends on the temperature field beside it), and **time step** (blank = 0.05 s; see below). Values display in your preferred units and convert to SI on Launch. When temperature and pressure are blank the sim uses the ISA atmosphere (288.15 K, 101325 Pa, −6.5 K/km lapse).

One field is worth a second look now that thrust depends on it too. **Station pressure** is the pressure *at the pad* — what a barometer reads standing there — not the sea-level-corrected altimeter setting an airport broadcasts. The field used to be labelled just "Pressure", which left every reader to supply their own meaning, and the common one is the wrong number by 15 % at 3,900 ft. Two ways to get it wrong, and both matter more at a high site:

- **Leave BOTH temperature and station pressure blank** and the app computes the pad's pressure from your site altitude, which is the right answer. But **leave only the pressure blank while you type a temperature** and it does not: the two are read as a pair, and the app then flies **101,325 Pa — sea-level pressure — at your pad**, whatever altitude you set. Measured at a 3,900 ft (1,190 m) site: both fields blank gives 87,854 Pa at the pad; a temperature typed with pressure blank gives 101,356 Pa. At 8,800 ft the pair is 73,018 Pa against 101,354 Pa — the same sea-level number. **So whenever you type a temperature, type the pad's station pressure too.**
- **Typing about 1,013 hPa** at a 3,900 ft site is the same mistake by hand: you have told the app the pad is at sea level.

Either way the air comes out too dense (roughly 15 % at 3,900 ft) and the motor loses the pressure-thrust credit that thin air should give it (see *Multi-Stage and Clustered Rockets → Nozzle exit diameter*). **This is the single biggest thing you can do to make an imported RASAero file fly honestly at altitude**: RASAero writes a temperature into every file and a pressure into fewer than half of them — of the 60 tester files here, all 60 state a temperature and 25 state a pressure, and 24 of the 33 flown from 1,000 ft or higher state none. Some of the files that do state one carry an altimeter setting rather than a station pressure, which is the second mistake above, made inside RASAero.

You do not have to spot either one yourself. Above a 600 m (about 2,000 ft) site the panel says so in place — under the fields, with the pressure your site altitude implies, so you have a number to type — and **an imported RASAero file that lands in either mistake gets one line in its import note** naming the site's own standing pressure. Below 600 m neither says anything: the difference is under 7 % of air density there, and a caution that fires on almost every file is one you stop reading. Both go quiet the moment the pad's pressure is plausible for its altitude, so a station pressure you measured — or one an earlier RASAero file already carried correctly — is never second-guessed.
...
**How big is it?** It is **zero at liftoff and grows the whole way up** — and at liftoff it is zero *whatever your site altitude*, unless you have typed the pad's own station pressure. That is worth stating plainly, because it is the opposite of what a high-site flyer expects: a file that leaves the pressure blank is flown at 101,325 Pa **at its own pad** (see *Launch Conditions* above), so the term starts at nothing and the whole gain comes from climbing. Measured on a tester's file — G record 2023, a 0.45 inch (11.4 mm) exit on an 8,800 ft pad — the app as it imports the file gains **−0.003 N** on the pad; type that pad's station pressure (about 730 hPa) and the same file gains **+2.91 N** there instead. Type your station pressure — and on a pad that high, the import note and the Launch Conditions panel will both ask you to.
...
`A_exit` is the area of the per-stage **nozzle exit diameter** you type under the stage, and `P(h)` is the pressure the ISA profile above hands the drag term at that same instant — thrust and drag always see one atmosphere, so the site's typed temperature and pressure flow through both. (Which is why the **Station pressure** field matters here: with a temperature typed and that field blank the app puts 101,325 Pa at your pad, and the term is then zero at liftoff however high the site — see *Launch Conditions*, where the panel and the RASAero import note both now say so on a high pad.) The term is added **once per thrusting stage**, **only while the curve's own thrust is above zero**, and only under the **Rogers Kbf**, **Auto** and **Supersonic** models, which is the same gate the power-on base drag has always used (see *Multi-Stage and Clustered Rockets → Nozzle exit diameter*). Two visible consequences: the **Thrust** plot sits above the published curve while a stage burns — by nothing at a sea-level pad, by up to the exit area times full sea-level pressure near vacuum — and the launch report's **thrust:weight at rod departure** reads from the same value, so at a high site it moves too.

**The sea-level reference is an assumption, and this is its size.** No thrust-curve file records where it was fired: neither the RASP nor the RSE format has a test-site field, and the motor database has no nozzle or test-pressure column. So the app assumes every curve is a sea-level curve, and cannot correct one that was really measured on a mountain. The cost of that is a constant offset, and it is small: a motor with a 1 inch (25 mm) exit fired at a 1,300 m stand is overstated by about **7.4 N — 1.4 % of a J540R** — at every altitude, and by about 10 N at a 1,782 m stand (1.8 % of a J540R, 2.5 % of a 400 N J). That bias sits in the app's numbers with or without this term, because a sea-level-referenced curve flown from a sea-level pad produces exactly the thrust it always did. Referencing the correction to the pad's pressure instead would be wrong for every curve that genuinely was measured near sea level, which is most of them. The term therefore adds no new bias anywhere — it supplies the exact slope above the pad and leaves the existing offset where it was.
