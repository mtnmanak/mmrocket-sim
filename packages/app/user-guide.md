...
**1. Load a motor.** Open the **Motors & Launch** workspace (the tabs under the header) and find the **Motors** panel. The fastest option is the **Quick picks** dropdown — pick a classic Estes motor like **B6-4** or **C6-5** and it loads at once. (The number after the dash is the ejection delay in seconds.) These are ordinary catalogue motors with their certified thrust curves; their curves ship with the app, so nothing here needs a network. Want something specific? Click **🔎 Browse motors / import EX (.eng, .rse)…** to search the full thrustcurve.org catalog of {{MOTOR_COUNT}} motors — but for your first flight, a quick pick is all you need.
...
| ⧉ **Duplicate** | Copies the part and everything inside it, drops the copy in beside the original, adds *(copy)* to its name, and selects it. |
...
- `kind` **is matched exactly, capitals and all** — `BodyTube`, `NoseCone`, `Transition`,
  `CenteringRing`, `TubeCoupler`, `BulkHead`, `EngineBlock`, `LaunchLug`, `Parachute`, `Streamer`.
  An inner tube uses `BodyTube`. Type `bodytube` and the row imports and then shows up in no picker.
...
- **Fill in the three material columns together.** A row with a material name but no density, or a
  density but no name, is counted as imported and carries no material at all, so the part quietly
  keeps the material and weight it already had. A blank `materialType` is read as `BULK`, which on
  a canopy means its fabric weight is dropped rather than applied — write `SURFACE` for a parachute
  or streamer canopy and `BULK` for everything solid.
...
**The field that catches people is station pressure.** It is what a barometer reads *standing at the pad* — not the sea-level figure an airport broadcasts or a weather app shows, which is the number most readily to hand and looks exactly like a pressure. At a 3,900 ft pad a barometer reads about 878 hPa (25.94 in-Hg) on a standard day, against 1,013 hPa at sea level. Type the sea-level figure there and the app flies air about 15 % denser than the pad's: more drag on the way up, and the motor loses the extra thrust thin air gives it (see *Multi-Stage and Clustered Rockets → Nozzle exit diameter*).
