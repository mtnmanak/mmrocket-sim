...
**A blank field.** A blank field that stands for one known value shows that value in grey as *default: N* — a fin set's cant of 0, a new canopy's 6 shroud lines, a deployment altitude of 200 m — and the spinner and the arrow keys step from it. A blank with no single value behind it, such as the automatic Cd, the standard time step or a plugged delay, stays blank, and the spinner leaves it alone.
...
after a pod's or a strap-on's, because the weighing rides with the core's motor. A design autosaved by an
older version with its weighing on a pod's or strap-on's motor has it moved onto the core's when the app
reopens it, and a note says where it went; the number itself is unchanged. Changing the delay keeps the weighing, because the hardware does not
...
Each **stage** has its own "Max motor length" — a booster and a sustainer have different room. Motors longer than the limit are flagged in the browser and excluded from batch simulation. One limit serves every mount in the stage, the mounts in its pod sets and strap-ons included, so **⌾ Estimate** gives the tightest of their estimates. This is separate from the diameter-fit rule; a motor can be the right diameter but too long.
...
| **.csv / .xlsx** (component data) | — | Yes | Every component as one row — dimensions, material, shape, and the engine's computed mass/CG/position — in your preferred units. A **Copies flown** column says how many of each part fly, every pod, strap-on and cluster copy counted, and both mass columns count all of them; the **Stage** column files a strap-on's parts under the strap-on. For sharing measurement data with people who don't run a simulator. |
...
**Every tab shares one autosave.** Open the app in two tabs of the same browser and they write to the same autosave. When another tab has changed it, this tab stops autosaving rather than write over that work, and a banner says *This design was changed in another tab*, with two choices: **Keep this tab's design** autosaves this tab's design over the other's, and the other tab then shows the same banner; **Load the other tab's design** reloads this tab from the autosave and discards its unsaved changes. Until you choose, nothing this tab does is kept, so closing it asks first.
...
**Where Undo stops.** An **Open**, a **share link** and a **flight-configuration switch** each start the undo history over, so Ctrl+Z never reaches back across them to a design flown under other motors and conditions — which also means switching configuration clears the undo steps you had. **✕ New** does not: Ctrl+Z still brings the components back, though not the motors or the Measured mass & CG box. The undo and redo keys do nothing while a dialog is open (the first-run tour does not count as one), and Undo and Redo do nothing at all while a flight is being run. After a motor change, an undone state keeps the nozzle exit of the motor loaded now, not the one it was recorded with.
