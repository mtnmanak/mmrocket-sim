...
The second one the app now fills in for you where it can. Since v0.122 it ships **AeroTech's own published exit diameters** — 191 motors, read from AeroTech's assembly drawings and dimensioned nozzle spec pages, covering 189 of their 272 motors in production. Assign one of those motors and the **Nozzle exit diameter** field under the stage on the **Motors & Launch** page fills itself in, with a line underneath naming the nozzle part number and the drawing it came from. Three things worth knowing:

- **A value you already have is never overwritten.** If your design carries a nozzle — typed by you, or imported from a RASAero file — and it disagrees with AeroTech's, the app shows both numbers and offers a one-click accept. It does not change your data behind you. This is where the database earns its keep: one tester's file gave the N1000W a 2.737 inch exit where AeroTech's drawing says 1.750, which is 2.4× the area and most of why that file over-predicted its flight by nearly 60 %.
- **Nine motors have two published nozzles.** The app takes the one AeroTech's own dated revision block calls current and says so, naming the alternative — the K1100T's two options differ by 43 % in area, so check which nozzle is in your reload kit.
- **Only AeroTech, for now.** Loki and Cesaroni publish no nozzle geometry, so those motors leave the field alone and say nothing. Take the number from the motor's data sheet or measure the hardware; all the app can check is that it is not wider than the casings it comes out of.
