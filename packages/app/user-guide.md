...
The **roll slider** down the far left turns the rocket about its long axis — desktop OpenRocket's rotation slider, in the same place. Drag it, and every part that has a clock angle sweeps through the view: each fin is foreshortened by the cosine of its angle, so a three-fin set reads as one fin at full span and two at half. A fin pointing **toward** you is in front of the airframe and is drawn whole, crossing the tube; one pointing **away** is behind it and the tube covers its root. That is what tells the two lower fins of a three-fin set apart, and it is what lets you watch a fin cross from the far side to the near side as you roll. **While the view is rolled, the covered part of a far fin is also drawn as a hidden line** — the same dashed outline used for anything else inside a tube — so a fin turning edge-on can be followed all the way through the body instead of disappearing at the wall. That is how desktop OpenRocket's figure reads, because it draws every component as an outline and fills nothing. At rest the hidden lines are off and the drawing is the plain one. One thing to expect rather than puzzle over: **a set of N identical fins repeats every 360/N degrees** — a three-fin rocket rolled 120° is the same rocket, and the side view has no way to tell you which fin is which, so the picture comes back to where it started a third of the way round. Desktop OpenRocket does exactly the same. The Aft view is where you can follow one fin the whole way round. Pods, parallel boosters and motor clusters swing with it. The readout under the slider shows the angle and clicking it returns to zero (so does double-clicking the slider). It is a **view** control, like zoom: it changes nothing in the design, nothing in the numbers, and nothing is saved — but it is shared with the Aft view, so you can roll the side view and switch to Aft to see the same attitude from behind. Launch lugs, rail buttons, shrouds and protuberances do not roll, because the app does not yet store a clock angle for them; they stay drawn on the top surface.
