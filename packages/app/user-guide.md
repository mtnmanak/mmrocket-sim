...
Before you fly, the Launch Conditions panel (in the **Motors & Launch** workspace) sets the pad and the weather. It collects nine fields and shows one readout: **launch rod/rail length** (default 1 m), **rod angle** (0°, range −30…30°), **average wind**, **wind gust sigma** (standard deviation of gusts about the average), **site altitude** (0–10,000 m), the **density altitude** readout beside it (see below), **temperature**, **station pressure**, **latitude** (default 28.61°, roughly Florida), and **time step** (blank = 0.05 s; see below). Values display in your preferred units and convert to SI on Launch. A design file (`.ork` or `.CDX1`) carrying a rod, wind, site altitude or latitude outside a field's range has it brought inside, and a temperature or pressure no field would take is left blank so the site's standard air flies; the import note quotes the file's own number either way. The rod angle's ±30° is tighter than desktop OpenRocket's ±60°, so a steeper rail from a desktop file flies here at 30°.
