...
The second one the app now fills in for you where it can. It ships **the manufacturers' own published exit diameters** — 246 motors: AeroTech's, read from their assembly drawings and dimensioned nozzle spec pages (**188** of their 272 motors in production — a 189th has a row and no number, the J615ST aerospike, which has no exit plane at all), and since v0.127 **Loki Research's**, read from their reload-kit instruction sheets and the nozzle tables on their own Tech Info page (54 of their 58 in production). Assign one of those motors and the **Nozzle exit diameter** field under the stage on the **Motors & Launch** page fills itself in, with a line underneath naming whose figure it is, the nozzle, and the document it came from. Six things worth knowing:
...
- **Cesaroni publish nothing anyone has found**, and four Loki motors are short: their published exit table stops at 76 mm, so the 98 mm N3800 and N5500 have none, and the 54/4000 case (L2050, M1378) has no commercial nozzle listed because those nozzles are one-time-use. Those motors leave the field alone and say nothing. Take the number from the motor's data sheet or measure the hardware; all the app can check is that it is not wider than the casings it comes out of.
