...
Before you fly, the Launch Conditions panel (in the **Motors & Launch** workspace) sets the pad and the weather. It collects ten fields and shows one readout: **launch rod/rail length** (default 1 m), **rod angle** (0°, range −30…30°), **average wind**, **wind gust sigma** (standard deviation of gusts about the average), **site altitude** (0–10,000 m), the **density altitude** readout beside it (see below), **temperature**, **station pressure**, **latitude** (default 28.61°, roughly Florida), **longitude** (east positive, so every US site is negative; blank flies −80.6°, desktop OpenRocket's default, and it moves no flight number — it places the flight in the flight-data file's Longitude column and travels with the `.ork`), and **time step** (blank = 0.05 s; see below). Values display in your preferred units and convert to SI on Launch. A design file (`.ork` or `.CDX1`) carrying a rod, wind, site altitude, latitude or longitude outside a field's range has it brought inside, and a temperature or pressure no field would take is left blank so the site's standard air flies; the import note quotes the file's own number either way. The rod angle's ±30° is tighter than desktop OpenRocket's ±60°, so a steeper rail from a desktop file flies here at 30°.
...
**What it can and cannot tell you.** The wind is the forecast's 10 m (33 ft) wind at the hour you pick — an instantaneous value at that time, not an average over the hour — and at one level: the app's wind does not change with height. The gust is the strongest in the hour before, shown for reference (see *Wind gusts from a forecast* below). The wind direction is shown too, but not applied: the app's wind has no compass direction. Open-Meteo forecasts 16 days ahead at most; for past dates it keeps its forecasts for 92 days, and before that the app uses the ERA5 reanalysis — the weather as it was, back to 1940, not a forecast. A town search finds the **town centre**, not your field, and the review says so; for a field out of town, paste the field's coordinates. Postal-code search works mainly in the US and a few other countries — elsewhere, search the town name.
