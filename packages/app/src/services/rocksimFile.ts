  ...
  const reconstructClusters = (nodes: ComponentNode[]) => {
    for (const parentNode of nodes) {
      ...
      for (const g of groups.values()) {
        ...
        const weighedTubes = g.filter((t) => numOpt(t, 'overrideMass') !== undefined);
        const balance = new Set(g.map((t) => {
          const cg = numOpt(t, 'overrideCGX');
          return cg === undefined ? null : Math.round(cg * 1e4); // to 0.1 mm
        }));
