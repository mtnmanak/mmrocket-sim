 ...
 * names and states are read from the DOM, and the one behaviour — where focus
 * lands after Launch — is driven. Row 462, the stats drawer's two halves, is
 * App.render.test.tsx's; the notices' live regions are NoticeBar.test.tsx's.
 *
...
async function launch(host: HTMLElement): Promise<void> {
  ...
  await waitFor(() => (JSON.parse(localStorage.getItem(RUNS_KEY) ?? '[]') as unknown[]).length > 0,
...
describe('App — accessibility, as rendered', () => {
  ...
  it('row 443: Launch lands focus on the Results <main> and says the flight is done', async () => {
    const host = await mountApp();
    const region = document.querySelector('.sr-only[role="status"][aria-live="polite"]:not(.notice-announce)');
