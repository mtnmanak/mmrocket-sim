...
import { downloadBlob, schematicSvg, svgToPng, type ExportData } from '../services/schematicExport.js';
      ...
      <div className="schematic-controls">
        {exportData && (
          <>
            ...
            <button className="file-btn"
              title="High-resolution 2D image (PNG, 3840px wide) with design data"
              onClick={async () => {
                if (!svgRef.current) return;
                const data = { ...exportData, spanM: 2 * vHalf };
                const svg = schematicSvg(svgRef.current, scale, w, h, data);
                downloadBlob(await svgToPng(svg), `${data.name.replace(/[^\w-]+/g, '_')}-2d.png`);
              }}>
              ⬇ PNG
            </button>
          </>
