...
/**
 * AIRFRAME ONLY, again (v0.118). v0.116 put a third field here — the pad
 * weight with the motor in — and it was the wrong home: the two figures in
 * this box are meant to survive a motor change, a pad weight cannot, and
 * nothing said whether the button applied it. That field now lives under the
 * motor on Motors & Launch (components/MotorPadMass.tsx). These two pin the
 * box's return to its v0.115 shape, so the field cannot creep back.
 */
describe('MeasuredMassBox — airframe only, as in v0.115', () => {
  it('shows the v0.115 hint: with the motor out, nothing changes until you press the button', () => {
    show({ massKg: null, cgM: null });
    const hint = host.querySelector('.measured-hint')!;
    expect(hint.textContent).toBe('Weigh and balance the airframe with the motor out, then type what you got. Nothing changes until you press the button.');
    expect(hint.querySelector('strong')!.textContent).toBe('with the motor out');
    expect(host.textContent).not.toContain('with the motor in');
    expect(host.textContent).not.toContain('pad');
  });

  it('renders exactly two inputs', () => {
