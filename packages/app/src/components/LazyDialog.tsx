 ...
 * at v0.140 riding in the entry chunk, which every visit downloads and parses
 * before the app draws, for two dialogs most visits never open. Now each is a
 * chunk of its own, fetched the first time it opens (the service worker still
 * precaches both, so that works offline).
 *
 * Two things a static import never had to handle, and this does:
 *
 *  - THE DOWNLOAD TAKES TIME. While it runs, a stand-in dialog with the same
 *    name holds the place: it takes focus, answers Escape, traps Tab and holds
 *    the design's undo binding, exactly as the real one will (useDialog). When
 *    the real dialog arrives the stand-in unmounts first — React runs the
 *    removed tree's effect cleanups before the new tree's effects — so its
 *    cleanup hands focus back to the button that opened it, and the real
 *    dialog's own useDialog then records that button and moves focus inside.
 *    Close returns focus to the opener as it always did.
 ...
 * guide", as the real one does.
 */
export function LazyDialog({ label, onClose, children }: {
  label: string;
  onClose: () => void;
  children: ReactNode;
}) {
  return (
    <LazyDialogBoundary label={label} onClose={onClose}>
      <Suspense fallback={(
        <DialogStandIn label={label} onClose={onClose} busy>
...
/** A plain dialog with the real one's name, focus handling and ✕ Close. */
function DialogStandIn({ label, onClose, busy, children }: {
  label: string;
  onClose: () => void;
  busy?: boolean;
  children: ReactNode;
  ...
  return (
    <div className="prefs-overlay" role="presentation" {...backdrop}>
      <div className="prefs-dialog panel" role="dialog" aria-modal="true" aria-label={label}
...
> {
  ...
  override render() {
    ...
    return (
      <DialogStandIn label={label} onClose={onClose}>
        {this.state.download ? (
          <>
            <p>
