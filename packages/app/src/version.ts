...
export const CHANGELOG: ChangelogEntry[] = [
  {
    version: '0.135',
    date: '2026-09-19',
    title: 'Batch simulate was ignoring the nozzle data the Launch button uses, the G80 ignition rule was never a real rule, and the guide claimed a desktop match the app deliberately does not make',
    items: [
      'BATCH SIMULATE NOW FLIES EACH CANDIDATE ON ITS OWN NOZZLE, AND SOME OF YOUR BATCH NUMBERS WILL MOVE UP. Until now the sweep set the nozzle exit diameter aside for every candidate while the Launch button used it, so the same motor gave two different answers in two places - which is exactly the thing that costs you confidence in a tool. Each candidate now flies the published exit the app holds for it, summed across the stage as one equivalent nozzle for a cluster - the same exit the design page fills in for that motor, so the nozzle is no longer what makes the two disagree. If you typed your own exit under the stage and that motor is still loaded on the mount you are sweeping, its row flies YOUR value rather than the published one. Rows that flew a nozzle are marked in the Motor column: the app holds a published exit for AeroTech and Loki motors only, so on a typical sweep some rows carry the term and some do not, and the marker is there so that a few percent between two similar motors is explained rather than mysterious. Under Classic Extended Barrowman neither half of the nozzle model does anything, in the sweep or on the design page, and the marker is hidden with it.',
      'THE IGNITION DEFAULT FOR AN UPPER-STAGE MOTOR IS NOW ABOUT PROPELLANT, NOT POWER CLASS, AND THE OLD RULE WAS WRONG THREE WAYS AT ONCE. The app used to give a staged motor electronics-timed ignition when it was above the high-power line. That is not what decides it: a booster ejection charge lights a black powder grain and cannot light a composite one, whatever size either is - so a great many composite E, F and G motors were being defaulted to an ignition they cannot actually have. The old rule also read power class off ONE stage when it is a property of the whole rocket (four stacked F-15s make a high-power rocket out of four low-power stages, and every one of those stages still lights off the charge below it), and it only ever looked at the top stage, so a three-stage rocket got the wrong default in the middle. Now any motor above the launch stage defaults to booster burnout plus one second unless the catalogue records it as black powder, and a strap-on booster stays automatic whatever it burns because it lights at launch. You can override any of it, and no saved design is rewritten: this is the default a motor gets when you assign it.',
      'WHEN THE CATALOGUE RECORDS NO PROPELLANT AT ALL, THE APP ASSUMES YOU NEED AN IGNITER. That is 223 of the 1,156 catalogued motors - 97 of them still in production, and 90 of those 97 Contrail and Hypertek hybrids, which need an igniter in any case - plus every motor you import from your own .eng or .rse file, because neither format has a field for propellant. The two possible mistakes do not cost the same. Assume the charge will light it and the app flies a sustainer burn your hardware cannot produce, with nothing in the report to tell you. Assume it will not and the sustainer lights a second late, which you can see in the plot and change in one dropdown.',
      'A .CDX1 EXPORT NOW CARRIES THE NOZZLE EXIT DIAMETER, SO A ROUND TRIP THROUGH RASAero KEEPS IT. The format has fields for it in two places and this app has read both since v0.117, but the export wrote zero into all six of them - so a design sent out to RASAero II and read back came home without its nozzle, losing both the power-on drag reduction and the pressure-thrust term. Both are written now, in inches - the unit this format uses for every part dimension - with a booster stage that carries no engine left at zero the way RASAero writes it. If you export to RASAero, open one and check the value lands where you expect.',
      'A NEW CAMERA SHROUD IS TAPERED AT THE FRONT AND FLAT AT THE BACK. Real shrouds are flat on the end the camera looks out of; the app was creating them domed. Only NEW shrouds change - nothing you have already saved moves, because the file records the shape and the app reads what it records. A default shroud now carries a frontal drag coefficient of 0.65 where it used to carry 0.40.',
      'ON A PHONE THE NOTICE BAR NO LONGER SITS ON TOP OF THE TAB BAR AND EATS YOUR TAPS. It was painting over all four tabs and taking the taps meant for them, so the tabs and the Launch button under it simply did not respond. There was a rule meant to lift it clear, written in v0.074, that has never once worked: it and the rule it was supposed to beat carry identical weight, so the LATER one in the file won, and the later one was the wrong one. It has been shipping dead in all 61 releases since. Beside that fix, the autosave notice saying a design was read by an older build is now quiet rather than loud - it is advice, not a problem, and it appeared for every returning user whose autosaved design was restored, after every release, which is what kept forcing the bar open in the first place.',
      'IN DAYLIGHT MODE YOU CAN SEE WHICH MOTOR FILTERS ARE SELECTED AGAIN, AND THE ZOOM BOX ON THE PLOTS. A selected filter chip was carrying its state as an outline of the same black as its own border, over the same white background, so selected and unselected looked identical and picking motor diameters gave you no feedback at all. Selected chips are filled now. Separately, the drag-to-zoom rectangle on the flight plots was the charting library default faint grey, which measures 1.17 to 1 against the Daylight plot and 1.02 to 1 against the dark one - invisible in both. It is now drawn in the theme accent. It was worse in dark mode, where nobody had reported it, than in Daylight - an invisible selection box reads as a twitchy zoom rather than as a bug.',
      'THE FLIGHT-PLOT ZOOM NO LONGER RUNS AWAY, AND IT TELLS YOU HOW FAR IN YOU ARE. It was applying a fixed step per wheel EVENT while reading nothing but the direction, so a high-resolution or free-spinning wheel - which sends several events for one physical click - zoomed several times as far for the same turn of your hand. It now measures how far the wheel actually turned, so one click means one step on any mouse or trackpad, and the same rule now applies on the 2D and aft drawings. The step itself is gentler: seven clicks to double, twenty-two to reach ten times. Zooming in and then straight back out returns you where you started, instead of creeping 2.25 % inward on every in-and-out as it used to. A percentage sits beside the reset button, and the wheel stops at 5000 percent.',
      'THE LAUNCH BUTTON ON MOTORS AND LAUNCH NO LONGER DOES NOTHING. With no motor assigned it was fully enabled, and pressing it produced silence - no message, no navigation - while the vitals strip on the same screen showed a greyed-out Launch at the same moment. It is now disabled with no motor, and hovering it says why.',
      'A CHUTELESS BOOSTER IS NOW GRADED ON HOW FAST IT LANDS. The landing-speed check was written inside the branch for boosters that DO carry a recovery device, so one with none was never checked at any speed - only the separate must-recover-actively warning fired, and only above the high-power line. The speed was being computed all along; only the branching hid it.',
      'THE MOTOR CATALOGUE IS REFRESHED TO 19 SEPTEMBER, AND A STALE ONE WILL BE NOTICED FROM NOW ON. One motor added, none removed, no existing row changed. The pre-release check that watches third-party data used to allow a 30-day-old catalogue, a limit set before there was a weekly refresh; it now allows eight days, and it asks thrustcurve.org how many motors it holds, so a refresh that was fetched but never merged shows up instead of passing quietly.',
      'THE USER GUIDE HAS BEEN REWRITTEN, AND ITS FIRST PARAGRAPH WAS THE WORST OF IT. It said this app runs the real OpenRocket physics and had been verified bit-for-bit against the desktop program. The first half is true - the OpenRocket 24.12 kernel really is what computes your flight - but the second was not: the test it referred to compares this app own kernel on a JVM against the same kernel compiled to JavaScript, and there is no desktop program in it anywhere. Worse, the app does not fly that model by default. It defaults to Rogers Modified Barrowman, which adds terms 24.12 does not have, so a design brought over from the desktop is MEANT to read differently here. The opening and the whole How It Works section now say that plainly, and name the setting that gives the 24.12 answers back.',
      'AND THE REST OF THE GUIDE WENT THROUGH THE SAME PASS, AGAINST YOUR LIST OF NOTES AND ALONGSIDE YOUR OWN EDITS TO THE FILE, WHICH ARE IN THIS COMMIT TOO. Comparisons to desktop OpenRocket are gone except where a real compatibility fact needs stating. Version history is gone: the guide describes what the app does now, and the version chip in the header holds the history. Files are called .rkt files rather than RockSim files, because we do not know what program wrote them. Absolutes that were not absolute are softened, and ones genuinely true of this app are kept. Copy, cut and paste in the parts tree are documented for the first time, along with how they differ from duplicate. The custom-preset CSV round trip the guide claimed you could do is now actually explained. Reynolds number is defined. And the motor counts in the catalogue and offline sections are no longer typed in by hand - they are filled in from the shipped data when the guide is built, with the date beside them, so those cannot go stale between releases again.',
      'THE OFFLINE AND CLEAR-YOUR-DATA SECTION IS REWRITTEN FROM 27 DEVICE TESTS, AND IT HAD BEEN POINTING AT THE WRONG CHECKBOX. Clearing CACHED IMAGES AND FILES does not delete the offline copy - tested in Chrome, Edge and Firefox, cleared at All time, then reloaded with the network off, and the app opened every time. The warning belonged on COOKIES AND SITE DATA, which does take the offline copy, along with your current design, your saved runs, your preferences, your imported parts and your imported EX motors. Rebuilding the offline copy after that is not something one reload guarantees: in Firefox, going offline straight after the next online load still failed, and offline working came back only after a further spell on a connection.',
      'AND ON AN iPAD, USE SAFARI FOR FIELD WORK RATHER THAN THE HOME SCREEN ICON. Tested on one iPad running iPadOS 18.6.2: the Home Screen app is a SEPARATE copy with its own storage - it opens with a new empty rocket, a rename in one never reaches the other, and deleting the icon destroys everything that copy held. And it did not work offline, while Safari on the same device, in the same airplane mode, opened the same address and flew a simulation. That is the opposite of what anyone would assume. The cause is not established and the guide does not guess at one. One iPad, one OS version; no iPhone was tested and no Mac.',
      'SAVE AS .ORK DOES NOT QUITE KEEP EVERYTHING, WHICH MATTERS BECAUSE THE REST OF THAT SECTION TELLS YOU TO TRUST THAT FILE. Two things do not travel in an .ork, and both were checked in the code: a per-STAGE motor-length limit, and the curve of an EX or custom motor you imported from your own .eng or .rse. So a design flying an EX motor, archived and opened on another machine, arrives without that motor. Keep the original motor file beside the .ork. The guide used to promise the opposite.',
    ],
  },
  {
    version: '0.134',
  ...
  },
  {
    version: '0.130',
    date: '2026-09-13',
    title: 'Four numbers in the last three release notes were wrong, found by checking them',
    items: [
      'BEFORE WRITING THE FORUM POST FOR v0.127 TO v0.129 I HAD EVERY FACTUAL CLAIM IN THEM RE-CHECKED AGAINST THE CODE, ONE AT A TIME, BY SOMEONE WHO HAD NOT WRITTEN THEM - and then had each verdict attacked a second time. Seventeen claims went in. Four came back wrong, and all four had already shipped in a release note you can read in this app. They are corrected where they stand. NOTHING IN THE SIMULATION CHANGED and no number in the app moved; these were wrong SENTENCES about right behaviour, which is the fault this project has now shipped four times.',
      'WHAT WAS WRONG. (1) "A 1.818 inch exit is five and a half times the AREA of the 0.900 inch one" - it is just over FOUR times (2.60 against 0.64 square inches). Five and a half is the ratio against a different motor, the 0.780 inch I405. (2) "The v0.121 and v0.122 notes both recorded Loki as a gap" - it was v0.120 and v0.122; the v0.121 note says nothing about Loki. (3) The LEM-M2B measurement read plus 0.87 percent; 31.93 metres over 3,691.50 is plus 0.86. (4) "189 of AeroTech\'s 272 in-production motors have a published exit" - 188 do. 189 have a ROW, and the 189th is the J615ST aerospike, which has no exit plane for the pressure term to act on and deliberately carries no number. That last one was also in the user guide and is fixed there.',
      'AND ONE THING IN THE DATA, WHICH IS THE ONE THAT MATTERS. Loki publish TWO nozzle-throat columns side by side: what the commercial motor takes, and a suggestion for people making their own propellant. I transcribed that table from a flattened copy of the page, in which an empty cell simply is not there - so on all FIVE rows where the commercial cell is blank, the suggestion slid into its place. No motor in the catalogue uses any of those five cases, so nothing you could have loaded was ever affected, and the regenerated file is byte-for-byte identical. But a single new 76 mm motor from thrustcurve.org would have been given an experimental throat as though it were the factory one. The table now carries BOTH columns explicitly, a blank is written as a blank, and one accessor reads the commercial one. The file already carried a warning, in capitals, that these two columns are easy to confuse. I wrote that warning and then made the mistake anyway, which is the argument for a mechanism over a note.',
    ],
  },
  {
    version: '0.129',
  ...
  },
  {
    ...
    items: [
      ...
      'MEASURED, ON THE TESTER FILES THAT CARRY A LOKI MOTOR THE CATALOGUE RESOLVES, each flown twice through the kernel with nothing else altered. Mach 3 (M900-LR, 76 mm, a 1.818 inch exit): 7,153.89 m to 7,550.27 m, PLUS 5.54 PERCENT. LEM-M2B (J1026CT, 38 mm, 0.900 inch): 3,691.50 to 3,723.43 m, plus 0.86 percent. complexj (I405-LW, 38 mm, 0.780 inch): 1,449.47 to 1,456.96 m, plus 0.52 percent. The 76 mm number is the big one because a 1.818 inch exit is just over FOUR times the AREA of the 0.900 inch 38 mm one (2.60 against 0.64 square inches), and area is the whole of the term. Two-stage files are not quoted: measuring one properly needs its ignition configuration set, which is a separate sitting.',
  ...
  },
  {
    version: '0.127',
    date: '2026-09-13',
    title: 'Loki’s published nozzle dimensions, and the exit diameter now follows the motor',
    items: [
      'I SAID TWICE THAT LOKI PUBLISH NO NOZZLE GEOMETRY. THEY DO, AND IT WAS ON A PAGE WE HAD BOTH ALREADY READ. The v0.120 and v0.122 notes both recorded Loki as a gap. Their Tech Info page prints the nozzle exit diameter for every 38, 54 and 76 mm nozzle, banded by the number engraved on it - and it is at the FOOT of the page, under the O-ring table, which is why two passes over that page missed it. The owner found it. Both old notes are corrected where they stand.',
      'SO 54 OF LOKI’S 58 IN-PRODUCTION MOTORS NOW FILL THEIR OWN EXIT DIAMETER IN. The chain is Loki’s own, in two published documents: each reload kit’s instruction sheet names the nozzle that motor takes ("Important! The correct nozzle must be used or motor failure may occur"), the engraved number IS the throat in 64ths of an inch, and the Tech Info table gives the moulded exit for the band that number falls in. 43 motors are named on one of the 21 instruction sheets the owner supplied; the other 13 take the nozzle Loki’s own "Commercial Nozzle throat" column lists for that case, and every row says which of the two it came from. THE TWO SOURCES AGREE 43 TIMES OUT OF 43, and the build refuses to write the file if they ever stop. Four motors are still short and are named as such: the exit table stops at 76 mm, so N3800 has its #64 throat and no exit and N5500 has neither, and Loki list no commercial nozzle for the 54/4000 case, which is L2050 and M1378.',
      'THE EXIT DIAMETER NOW FOLLOWS THE MOTOR - IT IS PER MOTOR, NOT PER ROCKET. Reported by the owner: taking the motor out left its exit diameter sitting in the field ("there is no motor loaded, how can there be an exit diameter?"), and loading a motor whose published exit disagreed raised a warning you had to notice and click, so a motor swap could be flown on the previous motor’s nozzle. Now: change the motor and the number changes with it; take the motor out and the field empties; and either way a line says what the removed value belonged to. A value that came in WITH A FILE is still not overwritten - that is a number you typed for the motor in that file, and the two-figure comparison and one-click accept are unchanged for it. Since v0.119 this number buys thrust as the air thins, so a leftover exit is a wrong apogee in the direction nobody checks.',
      'AND A CLUSTER IS FILLED AS ONE EQUIVALENT NOZZLE, WITH THE AREAS SUMMED. The field has always been documented that way and the automatic fill added in v0.122 was not doing it: it wrote ONE motor’s exit diameter in whatever the cluster count, so a four-motor cluster flew a quarter of the exit area it has, and a quarter of the pressure-thrust term. Four identical motors is twice one motor’s diameter. If a stage mixes motors and any one of them has no published figure the field is left blank rather than filled with a sum that is short.',
    ],
  },
  {
    version: '0.126',
  ...
  },
  {
    version: '0.124',
    date: '2026-09-09',
    title: 'Saying what the pressure-thrust term actually does, and correcting a claim about RASAero in the v0.119 note',
    items: [
      '"ADDED ONCE PER BURNING STAGE" WAS BEING READ AS "ADJUSTED ONE TIME", which is not what it means and not what the app does. The owner asked the question directly, which is how I found out the wording was doing this. The term is worked out AFRESH AT EVERY STEP the simulator takes while a motor burns - four times per time step, each at that step’s own altitude and its own air pressure - so the extra thrust grows continuously the whole way up. Measured on one 7-second burn from an 8,800 ft pad: 2.9069 N on the pad rising to 4.3392 N at 5,420 ft, a 49 percent increase across the burn. "Once per stage" was only ever about HOW MANY nozzle areas get counted at each step - one per burning stage rather than one per motor, so a cluster is not charged twice - and that is now said in those words wherever it appears. Nothing about the simulation changed; it always did this.',
      'AND A CLAIM ABOUT RASAero IN THE v0.119 NOTE WAS WRONG, corrected there and owned here. That note said the app DELIBERATELY differed from RASAero by adding the term only during the burn, where RASAero was reported to add it before ignition and after burnout as well. Measured since, on paired RASAero exports: the difference outside the burn is 0.00000000 N across 8,515 rows on two designs. It is not a deviation - the app matches RASAero exactly here. The claim had a named source and a citation and was never checked, which is the same fault v0.121 was about.',
    ],
  },
  {
    version: '0.123',
  ...
  },
  {
    version: '0.122',
    date: '2026-09-09',
    title: 'A blank temperature or pressure now means the standard value for YOUR SITE, the nozzle exit diameter moved next to the motor and fills itself in from AeroTech\'s drawings, and an engineering audit of the whole app',
    items: [
      'LEAVE TEMPERATURE OR STATION PRESSURE BLANK AND THE APP NOW USES THE STANDARD VALUE FOR YOUR SITE ALTITUDE, not the sea-level one. v0.120 found that the two fields were read as a PAIR — type one, and the other silently fell back to sea level at whatever height your pad sits, worth 29.7 percent of apogee on one 8,800 ft flight — and answered it by telling you to type both. That was the wrong half of the problem. This is a simulation of a flight that has not happened, so nobody knows what the barometer will read on the day, and asking for the pressure "at the pad" read like a request for data after the fact. The pad\'s standard pressure is the one number here the app can work out and you cannot look up. So each field now fills itself in from your Site altitude independently, and the box SHOWS you the number it is using in grey — change the altitude and both follow it. Type a value only to try a specific day\'s air. Designs that left BOTH fields blank fly exactly as before. Two of the three cautions v0.120 added are gone, because they were the app warning you about its own fallback; the one that remains is for a sea-level altimeter setting typed into the station-pressure box, which no default can rescue, and its fix is now simply to clear the field.',
      'THE NOZZLE EXIT DIAMETER IS ON THE MOTORS AND LAUNCH PAGE, AND IT FILLS ITSELF IN. It was always a stage property, reachable only by selecting the stage in the component tree — but the number belongs to the motor, which is where people went looking for it. It now sits under each stage beside its motors, and it is the same field, not a second one. Assign an AeroTech motor and the app fills it from AEROTECH\'S OWN PUBLISHED DRAWINGS, with a line underneath naming the nozzle part number and the drawing it came from. 191 motors are covered, which is 189 of AeroTech\'s 272 in production. IT NEVER OVERWRITES A VALUE YOU ALREADY HAVE: if your design carries a nozzle — typed by you, or imported from a RASAero file — and it disagrees with AeroTech\'s, the app shows both numbers and offers a one-click accept. That is where this data earns its keep: one tester\'s file gave the N1000W a 2.737 inch exit where AeroTech\'s drawing says 1.750, which is 2.4 times the area and most of why that file over-predicted its flight by nearly 60 percent. Nine motors have two published nozzles; the app takes the one AeroTech\'s own dated revision block calls current and names the alternative, because the K1100T\'s two options differ by 43 percent in area. Loki and Cesaroni publish no nozzle geometry, so those motors are left alone. [CORRECTED IN v0.127: Loki DO publish it - an exit diameter per casing and nozzle-number band, at the foot of their Tech Info page. 54 of their 58 in-production motors are covered from v0.127. Cesaroni still publish nothing anyone has found.]',
      'THE PRESSURE-THRUST TERM ADDED IN v0.119 IS NOW CONFIRMED AGAINST RASAERO\'S OWN OUTPUT, and it is the same term rather than merely the right size. The owner exported paired RASAero runs — the same design with the nozzle on and with it off, so the motor\'s thrust curve cancels exactly — on two designs. The difference between the runs matches the app\'s formula to within 0.0035 N over 704 rows during the burn. On the pad it is 2.9069 N against 2.9076 N predicted, which also settles that RASAero flies a pad at that pad\'s own pressure and not at sea level. It is recomputed at every step of the burn rather than applied once, so the gain grows the whole way up; each step counts one nozzle per thrusting stage rather than one per motor, uses the burning stage\'s own nozzle, and is exactly zero while the motor is not thrusting. Nothing changed in the app: this is the check, and it passed. One thing MY OWN NOTES had wrong is corrected — they recorded that the app deliberately differed from RASAero before ignition and after burnout. It does not; across 8,515 rows outside the burn the difference is 0.00000000 N, and the v0.119 note is corrected to match.',
      'A RE-FLIGHT COULD SILENTLY LOSE A SUSTAINER\'S IGNITION SETTING, AND IT WAS THE RE-FLOWN RESULT THAT GOT STORED. When you assign a high-power motor to the top stage of a staged rocket the app sets it to light at burnout plus one second, because nobody lights a high-power sustainer off the booster\'s ejection charge. But writing a motor onto the simulation replaces its whole configuration, and three places re-wrote the motor without putting the ignition back: the automatic-delay re-flight, "Show charts", and the flight-data CSV export. So on exactly the designs the setting exists for, ticking "auto (optimal)" re-flew the rocket lighting off the booster\'s ejection charge instead — and that second flight is the one stored in your run history, shown in the report, and written into a saved .ork. All of them now keep the ignition.',
      'FOUR SMALLER THINGS THAT PUT A WRONG NUMBER ON SCREEN. Clearing the tube choice in the Scale dialog re-scaled the design off the SMALLEST tube in the catalogue — on a 98 mm design the button read "Scale to 5.1 percent". Closing the motor browser while a motor was downloading still loaded it afterwards, changing your design\'s mass, centre of gravity and stability. A malformed .eng file imported a motor with no propellant weight, which then failed at fly time blaming thrustcurve.org for a file off your own disk. And a negative burn-time or impulse filter could be typed into the motor browser and saved, so it opened permanently showing "No motors match these filters" with no visible cause.',
      'GEOMETRY THAT COULD NOT BE PRINTED, AND ONE THAT COULD NOT BE DRAWN. A zero or negative wall thickness produced a "solid" enclosing no volume at all — the app checked that such a mesh was watertight, which it was, but never that it was a solid, so the download button handed you a file with nothing in it; a negative thickness was worse, reporting a volume 72 percent too large. Both are now refused. A fin set with zero thickness, or a freeform outline that crosses itself, is skipped in the 3D view and the STL rather than drawn as a shell with holes in it. And any dimension that was somehow not a number — the app had twelve separate readers for those, and eleven of them treated "not a number" AS a number — could fill a downloaded STL with meaningless coordinates and make the part unclickable in 3D. There is one reader now.',
      'KEYBOARD AND SCREEN-READER FIXES THROUGHOUT. The component tree announced itself as a tree, which promises arrow-key navigation, and had none — while every row was a separate tab stop, so a 40-part rocket took 40 presses to walk past. Arrow keys, Home and End now move the selection, and one row at a time is tabbable. The fin planform editor and the aft-end view were both marked as images, which hides everything inside them: in the aft view that silently suppressed the name of every part shown. The preset picker had no column headers at all. The delete button in the run history announced as "times button". Several results that appear after you wait for something — an export failure, the measured-mass verdict, a preset import — were never announced at all.',
      'AN ENGINEERING AUDIT OF THE WHOLE APP, and what it found. Five reviewers read every part of the front end; 73 findings, each re-checked against the source before it was accepted. Everything above came out of it, along with three hardening fixes on files that arrive from other people: a design file could freeze the tab for minutes through a quirk of how comments were scanned, certain reserved words used as values in a file could bypass the app\'s "I do not recognise this" checks, and a fin outline with an absurd number of points had no limit. The full record, including what was deliberately NOT done and why, is in the repository. Under the bonnet: about 90 new tests, two shared helpers where there had been duplicates that had already drifted apart, and the build now enforces seven more rules that were measured to cost nothing today.',
    ],
  },
  {
    version: '0.121',
    date: '2026-09-08',
    title: 'Corrections to yesterday\'s release: three claims in the v0.120 note were wrong, and the stated-launch-weight fix had holes in it',
    items: [
      'THREE THINGS THE v0.120 NOTE CLAIMED THAT WERE NOT TRUE, corrected in that note and owned here. It said the blank-pressure behaviour was "desktop OpenRocket\'s own rule": it is not. On the RASAero import path desktop sets those two fields and then never uses them, because it leaves its standard atmosphere switched on, so it flies the site\'s own standard air and ignores both. What the app does is its own choice, and a defensible one, but it is not desktop\'s. It said the nozzle data covered "every 98 mm motor": it covers 28 of AeroTech\'s 32 in-production 98 mm motors. The M1305M, M1340W, N1975W-PS and O5500X-PS have no row, because the paperwork this data is built from is the reload-kit assembly drawing and those four do not have one. And it said the launch-weight correction "holds on every route a motor arrives by", which is true only for designs imported since that release: a design imported earlier and saved as an .ork carries no record that its stage weight still holds a motor, cannot be given one, and still counts that motor twice. RE-IMPORT SUCH A DESIGN FROM THE ORIGINAL RASAERO FILE to get the correction. That limit is now stated in the v0.120 note as well.',
      'THE STATED-LAUNCH-WEIGHT CORRECTION SUBTRACTED THE WRONG MOTOR. It took whatever motor you loaded out of a stage weight that contains a different one. A RASAero file can state a different launch weight for each of its simulations, so switching to another simulation and loading its motor wrote an airframe mass that was far too heavy: on one tester\'s eight-simulation file, switching from the first simulation to the sixth wrote 36.8 lb as the airframe against a true 14, and the rocket flew at 47.0 lb against that simulation\'s own 24.2. The correction now applies only when the motor you load is the one the file named for that stage. Load a different one, or switch to another simulation, and the app clears the stage\'s mass and centre-of-gravity overrides and falls back to the computed geometry, with a note naming both motors and quoting the file\'s own figure so you can type it back. A KNOWN LIMIT WITH IT: the app cannot derive another simulation\'s stated weight, only decline to use the old one.',
      'FIVE MORE WAYS THE SAME CORRECTION WENT WRONG, ALL CLOSED. Undo reconciled against whichever motors were mounted at the moment you pressed it rather than the ones the step was taken with, so a Ctrl+Z after swapping motors could move a stage\'s mass by tens of pounds with nothing on screen, while redo landed on the right answer. Scaling a design multiplied a stage weight that still held a motor by the cube of the scale and kept the marker, so the correction then subtracted an unscaled motor from a scaled figure. Typing your own measured mass into such a stage kept the marker, so the app later subtracted a motor from your measurement and blamed the file for a number it never stated; the Overrides panel now says plainly when a stage\'s figures came from the file and still hold a motor, and typing your own clears that. A stage with two mounts corrected the wrong one depending on the order you loaded them. And a stage marked on its centre of gravity alone lost the marker when saved and reopened, so the file was written with something the reader threw away.',
      'THE TEMPERATURE HELP POINTED AT THE TRAP THE RELEASE WAS ABOUT. v0.120 told you to type the pad\'s station pressure. Doing that with the temperature left blank puts the standard 15 C AT your pad instead of the colder air a high site really has, which at 8,800 ft is 6 percent too little air density, and no caution fired because the pressure looked fine. The help now says what leaving each field blank really does, the caution covers both halves, and an imported RASAero file that gives a pressure but no readable temperature says so.',
      'AND THE PAD-PRESSURE FIGURE COULD READ "NaN". The standard-atmosphere formula behind the new caution and import note was only valid up to about 36,000 ft, gave figures up to 14 percent wrong above that, and produced nothing at all above 145,000 ft. A balloon-launched or very high site therefore quoted a meaningless pressure. It now models the atmosphere properly through the stratosphere and refuses to quote a figure outside the range it can stand behind.',
    ],
  },
  {
    version: '0.120',
    date: '2026-09-08',
    title: 'A file that names no pad pressure was flown at sea-level pressure at its own pad, a stated launch weight could count its motor twice, and AeroTech\'s own nozzle dimensions are now on file',
    items: [
      'IF YOU TYPE A TEMPERATURE AND LEAVE PRESSURE BLANK, THE APP WAS PUTTING SEA-LEVEL PRESSURE AT YOUR PAD. It computes the pad\'s pressure from your site altitude only when BOTH fields are blank; type just one and the other falls back to the standard sea-level value. Nothing said so, and it matters more since v0.119 because thrust now depends on the ambient pressure. Every RASAero file is affected: RASAero writes a temperature into all of them and a pressure into fewer than half. Of the 60 files testers have sent, all 60 state a temperature, 25 state a pressure, and 24 of the 33 flown from 1,000 ft or higher state none. Measured: at 3,900 ft the pad reads 87,854 Pa with both fields blank and 101,356 Pa with only a temperature typed; at 8,800 ft, 73,018 against 101,354. On one two-stage design that single empty field is worth 29.7 percent of apogee. The field is now labelled STATION PRESSURE, the pressure at the pad rather than the altimeter setting your field gives you, its help says what blank really does, a caution appears when the pad pressure is wrong for the site, and importing a RASAero file that states no pressure, or an altimeter setting, at a site above 600 m now says so and quotes the figure the site implies.',
      'A STATED LAUNCH WEIGHT COULD COUNT ITS MOTOR TWICE. A RASAero file states each stage\'s weight with the motor already inside it. When the app cannot identify that motor it applies the stated figure whole, which is correct while nothing is mounted and is what desktop OpenRocket does. But load that motor afterwards and its weight landed on top of a figure that already contained it. On the MESOS two-stage design that is 156 lb against the file\'s own 87, so the rocket flew a fifth of its real altitude, and the same trap sits behind ten motor designations across the files testers have sent. A stage now remembers that its weight still holds an unidentified motor, and the moment that motor is loaded the app takes its weight and its centre of gravity back out and says so in a note. That holds on every route a motor arrives by: the motor browser, opening a file, switching flight configuration, and undo. ONE LIMIT, STATED: a design imported before this release and saved as an .ork carries no such mark and cannot be given one, so it still counts the motor twice. Re-import it from the original RASAero file to get the correction. The centre of gravity was double-counted by the same mechanism and is fixed with it. Where the correction cannot be made safely the app clears the override rather than guessing, and says that too.',
      'AEROTECH\'S OWN NOZZLE DIMENSIONS ARE NOW ON FILE, 191 MOTORS, FROM THE MANUFACTURER\'S DRAWINGS. Since v0.119 the nozzle exit diameter changes your flight, and no motor database publishes one, so until now it had to be typed from whatever you could find. Every one of AeroTech\'s 324 published motor assembly drawings has been read, joined to its nozzle part drawing and to the store page for that part, and checked against the certifying body\'s own measurements where they exist. That is 189 of AeroTech\'s 272 motors still in production, and 28 of their 32 in-production 98 mm motors, each row carrying the drawing it came from and how confident the reading is. NOTHING IN THE APP USES IT YET. This release only puts the data in place, because how it should reach you is a separate question. What it already shows is why it was needed: a tester\'s RASAero file gives the N1000W a 2.737 inch exit, and AeroTech\'s own drawing for that motor says 1.750, which is two and a half times the area and most of why that file over-predicted its flight by nearly 60 percent. Cesaroni publishes no nozzle data I could find anywhere, and Loki\'s is not published either; both are recorded as gaps rather than guessed at. [CORRECTED IN v0.127: Loki DO publish theirs, on their own Tech Info page and on every reload kit\'s instruction sheet. I had read that page and missed the table at the foot of it. 54 of their 58 in-production motors are covered from v0.127.]',
      'BATCH SIMULATE NOW SAYS WHEN EVERY ROW IS HEAVY. If a stage still holds an unidentified motor\'s stated weight, every candidate in a sweep flies on top of that weight. The dialog cannot subtract it, because it does not know what the named motor weighs and each candidate would need a different subtraction, so it says so plainly rather than presenting the comparison as though the weight were not there. Load the named motor first and the sweep is clean.',
      'WHAT THE VALIDATION SHOWED. v0.119\'s pressure-thrust term was checked against RASAero II itself, on flights run by hand in both programs, from a 10,000 ft G-motor flight to a 293,000 ft two-stage. Comparing change against change, which cancels the two programs\' aerodynamic differences, the app\'s combined nozzle model reproduces RASAero\'s to between 89 and 106 percent on four comparisons across two designs, and to within 4 percent on the big flight. The term is the right size. Two corrections to what earlier notes claimed: the nozzle\'s drag half was described as worth under 0.6 percent of apogee, which was one rocket\'s number, and on a design whose nozzle covers half its base it is 12 percent; and a stored RASAero result is not evidence, because one file in the set stores a figure 4.2 percent below what re-running that same configuration produces.',
    ],
  },
  {
    version: '0.119',
    date: '2026-09-08',
    title: 'Motor thrust now rises as the air thins, the way RASAero has always modelled it',
    items: [
      'A PUBLISHED THRUST CURVE IS A SEA-LEVEL MEASUREMENT, AND THE APP NOW FLIES IT AS ONE. The motor was fired on a stand with the whole atmosphere pressing back on its nozzle exit; as the rocket climbs the air pushes back less and the motor gains exactly the exit area times the pressure it has lost - F(h) = F_curve(t) + A_exit x (101,325 Pa - P(h)), Chuck Rogers\'s own formulation, and the term this app did not have. It uses the per-stage NOZZLE EXIT DIAMETER you already type under the stage (the number v0.117 started importing from RASAero files), it is added once per stage while that stage\'s motor burns, and it follows the aerodynamics model exactly as the power-on drag reduction always has: live under ROGERS KBF (the default), AUTO and SUPERSONIC, off under CLASSIC EXTENDED BARROWMAN, which stays bit-identical to desktop OpenRocket. Leave the nozzle at zero - which is every design that has never named one, every .ork and every RockSim file - and nothing about your flight changes.',
      'WHAT IT DOES TO NUMBERS, MEASURED. Twenty of the RASAero designs testers have sent carry a nozzle and a motor the database can resolve; each was flown twice through the kernel - same design, same motor, same launch conditions, this version against the last - and apogee moves from +0.007 % to +29.7 %, median +0.27 %. Twelve of the twenty move by less than half a percent (EZI65Hist-1 +0.007, the LEM-M2B family +0.02 to +0.10, ninja_4in_54mm-MMT +0.07, Level 3 Rocket +0.12, Wildman2Stage +0.43, EZI65Hist-2 +0.49). Four move by more than five: G record 2023 +5.5 %, 38-54 2-stage +5.9 %, 2,4-D +9.6 %, and OR vs RAS Test 1 +29.7 %. The big movers are all the same shape - a large exit on a slim airframe going high. IF YOU FLY ONE OF THOSE, RE-OPEN IT AND RE-FLY IT. Two ways back to the numbers you had, both proven rather than asserted: pick Classic Extended Barrowman in the vitals strip, which is unchanged bit for bit; or clear the stage\'s nozzle, which reproduces the previous flight to the last decimal on all twenty (it drops the power-on drag credit with it).',
      'AND A FINDING THAT MATTERS MORE THAN THE TERM ITSELF: A FILE THAT STATES NO LAUNCH PRESSURE IS FLOWN AT SEA-LEVEL PRESSURE AT ITS OWN PAD. Leave BOTH temperature and pressure blank and the app computes the pad\'s pressure from your site altitude, correctly. Type a temperature and leave pressure blank - which is what every imported RASAero file does, because RASAero writes a temperature into all of them and a pressure into fewer than half (60 of 60 tester files state a temperature; 25 state a pressure; 24 of the 33 flown from 1,000 ft or higher state none) - and the app puts 101,325 Pa at your pad however high the site. Measured: at 3,900 ft, 87,854 Pa with both blank against 101,356 Pa with a temperature typed; at 8,800 ft, 73,018 against 101,354. So the new term starts at zero on the pad for most imported files and the whole gain comes from climbing. On G record 2023 (a 0.45 in exit, 8,800 ft pad) the pad term as the file imports is -0.003 N; type that pad\'s station pressure, about 730 hPa, and it is +2.91 N. TYPING YOUR PAD\'S STATION PRESSURE IS NOW THE SINGLE MOST USEFUL THING YOU CAN DO TO AN IMPORTED FILE. The guide\'s Launch Conditions section says it in full.',
      'THREE SMALLER THINGS THAT COME WITH IT. BATCH SIMULATE now sets the design\'s nozzle aside and flies every candidate on its published curve - spending your rocket\'s nozzle on a motor that does not have one would decide the comparison with a number belonging to none of the candidates - so a batch row reads LOWER than the same motor flown from Launch: measured across those 20 designs, under 1 % of apogee on eleven and under 2 % on fourteen, but 7.9 to 45.8 % on the six with a large exit on a slim airframe. Compare batch rows with each other, not with a design-page flight. A PLAUSIBILITY CHECK now warns when the exit diameter you typed is wider than the motor it has to come out of (for a cluster, wider than all the loaded casings taken together, since the field is the single equivalent nozzle with the exit areas summed) - the field takes 1 to 200 mm and nothing checked it while it only trimmed drag. And a STORED FLIGHT of a nozzle-bearing design flown before this update no longer reports itself as matching the design as it stands: its numbers came from different physics, so Show charts will not re-fly it and an .ork export will not write it as that configuration\'s result. Re-fly it and it is current again.',
      'WHERE THIS DIFFERS FROM RASAero, stated rather than hidden. (CORRECTED 2026-09-09: this entry used to say the app DELIBERATELY differed from RASAero before ignition and after burnout, because RASAero was reported to apply its term outside the burn. Measured since, on paired RASAero exports - the same design with the nozzle on and off, so the motor curve cancels - the difference outside the burn is 0.00000000 N across 8,515 rows on two designs. It is not a deviation: the app matches RASAero here. The claim had a named source and a citation, and was never checked.) The app adds the term only while the motor\'s own curve thrust is above zero - the same instants the power-on drag reduction switches on and off, so the two halves of the nozzle model always agree. The corrected thrust is also floored at zero, which RASAero does not do: the term goes slightly negative on a pad above 101,325 Pa (correct, and tiny), and a wildly oversized exit typed against such a pad could otherwise drive a burning motor below zero, which is not a thing a motor does. The full exit area assumes a full-flowing nozzle, so tail-off is slightly over-credited, and a cluster whose motors are airstarted at different times is credited the whole summed area from the first ignition. The sea-level reference is an assumption: no thrust-curve file records where it was fired, so a curve genuinely measured on a mountain carries a constant offset the app cannot see - about 7.4 N, 1.4 % of a J540R, for a 1 inch exit fired at 1,300 m - and this term neither adds to that nor removes it.',
    ],
  },
  {
    version: '0.118',
    date: '2026-09-08',
    title: 'The weighed pad mass moves to the motor it was weighed with, batch simulate carries it, and a motor edit survives a configuration switch',
    items: [
      'THE WEIGHED PAD MASS NOW LIVES WITH THE MOTOR, ON THE MOTORS & LAUNCH TAB - NOT IN THE MEASURED MASS & CG BOX. v0.116 put it beside the two airframe figures, which are meant to survive a motor change, and a pad weight cannot: it is one rocket with one motor and that motor\'s adapter, retainer and closure in it. So unloading the motor left the number showing, loading a different motor left it showing and refused it in small print, and nothing said whether the Update button applied it. That shape was wrong and it is gone. The field, "Weighed pad mass with this motor", now sits under the loaded motor on the top stage\'s mount with its own line beneath, and the line says what it carried, which motor it was weighed with, and that the next Launch flies it. Unload the motor and the number goes with it; load a different motor and the field is blank for that motor, its placeholder showing what the app assumes (dry mass plus catalogue motor) until you weigh again; a delay or plugged change on the same motor keeps the weighing; each flight configuration keeps its own; and on a staged rocket, swapping or removing a booster motor, loading a mount that was empty, or changing a cluster count greys the number out and stops it being applied - the line names the motor it was weighed with and the one now there - until you re-weigh or put the set back. The arithmetic is unchanged - pad minus dry minus catalogue motor, carried on the motor for the whole flight so the pad mass is exact and the adapter comes down with the rocket - and a design with a pad mass entered flies the numbers it flew yesterday: both owner-flown files, re-flown through the kernel under v0.117 and under this version, agree on every one of 466 stored values.',
      'IT ACTS THE MOMENT YOU TYPE IT AND IS USED EVERYWHERE A FLIGHT IS FLOWN - the design page, Launch, and now BATCH SIMULATE, on the weighed motor only: every other candidate is a motor you have not weighed, so it flies at its catalogue weight, and the batch note names the row that carries the hardware. A CORRECTION to the v0.116 note and guide, which said the hardware was worth "about 1.5 percent" of apogee: that figure traced to no measurement. Measured through the kernel on the two flights this came from, with the pad weighed and the file\'s own launch conditions, entering the pad weight LOWERS the predicted apogee - the Wildman 4 inch Extreme on a J460T carries 129 g and drops 2.78 percent (1,655.8 to 1,609.9 ft) with 2.0 percent off peak velocity; the Monster Mamba on a J540R carries 182 g and drops 2.98 percent (1,502.1 to 1,457.4 ft). The batch row for the weighed motor moves by exactly the same amount, because it flies the same spec.',
      'A MOTOR EDIT NOW SURVIVES SWITCHING FLIGHT CONFIGURATIONS. Until now a delay or ignition change made on one configuration was lost when you applied another and came back, and it reached the saved file only if you saved while that configuration was still on screen - true since flight configurations arrived in v0.050. The working set is now written back into its configuration when you switch away and when you save, so the pad mass, and the delay edits that had the same problem, stay with the configuration they were made on. Two consequences, stated: pressing Apply on the configuration already on screen now keeps your edits rather than reverting them to the file\'s (its tooltip says so), and after a save, switching away and back is no longer reported as unsaved work. A reload also no longer loses a configuration\'s unmatched motor references, which v0.117 dropped from the saved file.',
      'IF YOU ENTERED A PAD MASS IN v0.116 OR v0.117, THE APP MOVES IT FOR YOU. On the first open after this update the value is checked against the motor you have loaded: when it can be that motor\'s pad weight it is placed under that motor and a notice says so - check the line under it, and the app asks you to save once, because the value moved; when it cannot (lighter than the dry rocket plus that motor, which means it was weighed with a different one) it is dropped and the notice names the value and the motor, so nothing is ever shown refused under a number that looks live. A file saved by those versions still opens: its pad mass goes to the motor on the top stage\'s mount in the file\'s default configuration, is checked the same way, asks you to save once, and saves in its new place next time - a rocket-level tag that names its configuration, which desktop OpenRocket skips with the same one-line warning it gives the two measured airframe figures. A design that never entered a pad mass writes the same file it wrote before: five tester files exported under v0.117 and under this version are identical byte for byte. Limits that remain, stated: the whole difference is carried on the top stage\'s mount, never split across a booster\'s; a delay or plugged change on the same motor keeps the weighing, so if you change the forward closure, re-weigh; a motor whose thrust file has no mass column cannot have its hardware separated from it; RockSim and RASAero exports do not carry it; the batch\'s mixed-cluster combination rows fly at catalogue weight; a configuration that is not the one on screen exports its stored flight results only if they were flown with no pad mass; and a file whose motor could not be matched keeps its pad mass on that motor\'s reference, unapplied, until that motor is loaded - loading it takes the value with it, and loading a different motor there drops it with a note.',
    ],
  },
  {
    version: '0.117',
  ...
  },
  {
    ...
    items: [
      ...
      'THE FIX RETIRES THE STORED CURVES RATHER THAN TRYING TO REPAIR THEM. Refreshing to v0.111 discards every curve the app had downloaded, so the next motor you pick is resolved against the current rule. Nothing else in your browser is touched - the design that saves itself, your saved runs and your preferences are all left alone.',
      'ONE THING A REFRESH CANNOT DO, AND IT IS THE CASE MOST PEOPLE ARE IN: the design you already have open keeps the motor it already resolved. A loaded motor is stored WITH the working design, curve and all, and it is restored exactly as it was saved rather than looked up again - so clearing the downloaded curves does not reach it. To pick up the corrected curve on a design you have open, RE-PICK THE MOTOR in the motor browser, or re-open the file. Re-picking is enough; it resolves the motor again, and now there is nothing stale left for it to find.',
  ...
  },
  {
    version: '0.107',
    date: '2026-09-05',
    title: 'The three hand-written motors are gone, every catalogued motor flies offline, and the v0.105 changelog is corrected where it was wrong',
    items: [
      'THE THREE HAND-WRITTEN MOTORS ARE GONE, AND THIS ENTRY OWNS WHAT THEY WERE. Since the project\'s first day the app carried an A8-3, a B6-4 and a C6-5 as thrust curves typed out by hand - eight points each, real published masses, invented curve shapes. The C6-5 integrated to 10.40 Ns against a certified 8.82. v0.105 fixed the importer that preferred them over the database but left them in the app as the "Quick picks (built-in, offline)" and as the motor a new design starts with. That was the wrong fix. They are deleted; there is no hand-written motor data anywhere in the app now, and nothing below the database stands in for a motor that cannot be loaded - the app says so instead.',
      'A CORRECTION TO v0.105\'S HEADLINE. It said "your Estes motors were flying the wrong thrust curve". The true scope: three designations - A8, B6 and C6, at any delay, because every file format writes the designation without the delay - on the file-import path only (.ork, .rkt, .CDX1, share links), for every maker of those motors (Apogee, Estes, Klima, Quest and SCR all got the same hand-typed curve). Estes A10, B4, C5, C11, D12, E9, E12, F15 and everything above them were never affected, and neither was anything picked from Browse motor database. The error was not one direction: the C6 read 15 percent high on impulse (the old curve was 18 percent above the certified one), the A8 read 25 percent LOW, the B6 5 percent high.',
      'EVERY MOTOR IN THE CATALOGUE NOW FLIES WITH NO NETWORK. The app ships every thrust curve thrustcurve.org publishes for every catalogued motor - 1,075 of 1,155 motors, 1,948 files, 840 kB, loaded on the first motor that needs it. A motor flies the same curve offline as online because the same rule chooses the file either way. The 80 motors with no curve are ones thrustcurve.org has no simulator file for at all - mostly out-of-production Gorilla loads and the new Jambol line - and they could not fly online either; import a .eng or .rse for those.',
      'A MOTOR WITH SEVERAL PUBLISHED CURVES NOW GETS THE RIGHT ONE. thrustcurve.org files the Estes A8-0 booster\'s curve (0.53 s, no delay, 2.14 Ns) under the same motor record as the A8-3/5 (0.73 s, 2.31 Ns), and the app chose between files by point count - the booster had two more points, so every Estes A8 flew about 7 percent low. The chooser now prefers a file whose burn time agrees with the catalogue\'s certified figure, then a certification-body file over a user upload, and only then the richer file. This protects all 1,155 motors, not three.',
      'THE MOTOR CATALOGUE IS REFRESHED. It was generated on 4 July and never updated. 26 motors are new, including a whole manufacturer (Jambol). AeroTech I218R\'s certified impulse is corrected 330 to 319.63 Ns. Cesaroni 2014K1200-16A is 488 mm long, not 404 - the app was saying it fit airframes it overruns by 84 mm. Seventeen availability flags changed, and Estes E12 is now marked out of production, which it is.',
      'THIRTY-NINE PARTS IN THE CATALOGUE HAD A DENSITY NO MATERIAL CAN HAVE, AND WEIGHED ACCORDINGLY. Eighteen paper centering rings and engine blocks from BalsaMachining and Rocketarium were declared at 0.0011 kg/m3 - a hundred-thousandth of air - and weighed nothing, which moves the computed CG forward and overstates stability on the small models that use them. Four Public Missiles fiberglass nose cones carried the balsa density; one of them with no catalogued mass computed 76 g for a cone its own sibling lists at 2.27 kg. A Rocketarium BT-50 motor tube was heavier than steel, two 29 mm tubes a decimal too light, two launch lugs likewise, and eleven LOC and Quest parts carried no density at all. All are transcribed faithfully from upstream files; upstream is wrong, and each is now corrected and watched. A plausibility screen on every bulk density now runs in the test suite, so the deploy fails if a regeneration ever brings one back.',
      'BLUE FOAM IS IN THE MATERIALS LIST. Styrofoam "Blue foam" (XPS), 32 kg/m3 - the one row of OpenRocket 24.12\'s 81 that the table had missed.',
      'A FILE WHOSE FREEFORM FIN OUTLINE CROSSES ITSELF IS NOW REFUSED ON IMPORT, WITH A NOTE. v0.105 said "both file importers already checked for this". They did not; only the fin editor did, and a crossing outline arriving in a .ork or .rkt still blanked the design. Both importers now run the same check as the editor, keep a default outline for that fin set, and say so.',
      'THE USER GUIDE NO LONGER TELLS YOU TO FLY A FINER TIME STEP TO CHECK ROD-EXIT SPEED. That advice was right until v0.102, which fixed the cause, and the guide still carried it.',
      'CORRECTIONS TO NUMBERS IN EARLIER ENTRIES, because a changelog that is wrong is worse than none. v0.105: a 250 g ballast 25 mm off centre raises roll inertia by about 0.03 gram-metre-squared (3.3e-5 kg m2), not "a third of" one - ten times less than stated. v0.104: the recovery weight of 3.38 kg is 41 percent lighter than the 5.74 kg pad weight, not 70 percent. v0.105: eighteen designation-and-diameter groups in the catalogue span more than one maker (22 by designation alone), not seventeen. v0.105: Save STL stopped downloading about 50 kB of 3D-viewer code, not 3.6 MB - that was the unpacked size of a library folder on disk, and three.js itself is still needed to write the mesh. v0.105: the audit produced 86 findings, not 84. v0.105: the ampersand bullet described a file the app could not produce - configuration NAMES were already escaped; it was the configuration ID that was not.',
      'TWO THINGS v0.105 CHANGED AND DID NOT SAY. A rail button was being positioned as though it were 25 mm long; the drawings and "Auto-place rail buttons" now put it where the physics flies it, so a design opened after v0.105 shows its buttons 12.5 mm (middle) or 25 mm (bottom) from where they were drawn before, and Auto-place lands the forward button on the CG as it always claimed to. And a recessed motor - a negative overhang, the ordinary retainer-cap build - was having its recess thrown away, so "Max motor length" over-reported the room by the recess depth and the motor browser offered cases that do not fit. Re-press the ⌾ button on any design with a recessed motor.',
    ],
  },
  {
    version: '0.106',
  ...
  },
  {
    version: '0.103',
    date: '2026-09-04',
    title: 'Six of the nine long-standing "not fixed yet" items — a stubby nose finally costs drag, a rail button flies the part you typed',
    items: [
      'A STUBBY NOSE CONE NOW COSTS DRAG. A Von Karman, Haack, parabolic or power-series nose was charged EXACTLY ZERO pressure drag below its drag-divergence Mach, at any fineness - so shortening one made the app predict LESS drag, because the only thing shortening changed was surface area. A cone was charged properly. Now they all are. Nothing changes above about 1.8 calibers of nose length, which is where almost every real design sits: a 3:1 or 5:1 nose does not move by a digit. Below that it climbs, reaching 0.12 of drag coefficient at half a caliber.',
      'WHERE THAT 0.12 COMES FROM, BECAUSE IT MATTERS. The obvious model - the one this app was about to ship - would have charged 0.42, and it is wrong. Centuri TIR-100 section 8 carries a wind-tunnel series nobody in the OpenRocket world seems to cite: twelve nose shapes on one Centuri Javelin, swapped one at a time, from 4 calibers down to a flat face. A hemisphere at half a caliber measures almost nothing there, and DeMar\'s NARAM-37 tests independently agree. A flat face costs 0.6 to 0.95. The lesson is that the penalty is about whether the nose is CURVED, not about how short it is - and a model keyed on the tip angle cannot tell a hemisphere from a flat plate, because they have the same tip angle. The number shipped is a third of what a cone of the same length is charged, fading out by 1.8 calibers. It is a bracket from measurement, not a calibrated curve, and the two tunnels involved state no test speed, no Reynolds number and no accuracy.',
      'A RAIL BUTTON NOW FLIES THE PART YOU TYPED, AND SAVING NO LONGER DESTROYS IT. Only the outer diameter ever reached the physics engine, so every rail button in every design flew as a generic 9.7 mm-tall part. Worse: saving a .ork wrote six hard-coded numbers over whatever the file carried - height, inner diameter, base and flange heights, screw height, and the material - so opening a design from desktop OpenRocket and saving it back REPLACED the builder\'s real button with a generic one. Three files on hand carry their own material density and were being overwritten with Delrin, up to 47 percent heavy. Height, inner diameter, base and flange are all in the same drag and mass formulas, so they move together. Two buttons on a test airframe contribute 0.0125 of drag coefficient at the 9.7 mm default; at 6 mm that is 0.0031 and at 15 mm it is 0.0401.',
      'A LAUNCH LUG, A RAIL BUTTON AND A DRAG BUMP NOW SIT WHERE YOU PUT THEM. The angle around the body never reached the physics engine at all for those three - it was set on the drawings and dropped on the way to the simulation, so every one of them flew at 180 degrees whatever you typed, and whatever the file said. A RockSim file\'s lug angle was not read either. This changes no drag - see below - but it does change the rocket\'s lateral balance and how it responds to wind.',
      'A DRAG BUMP\'S DRAG NOW VARIES WITH SPEED. A protuberance was measured once at Mach 0.3 and then charged that same number for the whole flight, which is not what this app\'s own user guide and property panel said it did. Measured on a test airframe, the frozen figure was 12 percent low at Mach 0.1, 18 percent low at the transonic peak and 49 percent HIGH at Mach 2. It now follows the rocket\'s own body drag at the current speed, which is what the method says. A camera shroud follows the same curve: its Mach 0.3 reading is unchanged, and only the shape with speed changes - about 14 percent more at Mach 0.1 and a third less at Mach 2. A protuberance you gave an explicit drag coefficient, and the flat-plate class, are untouched.',
      'A CAMERA SHROUD IN FRONT OF A FIN NOW GETS A SENTENCE. The shroud sheds a wake and the fin behind it is flown as though it were not there. Nothing in hobby software models that, here included, and no measurement sizes it - so the app now says so on the design tab when a shroud or a bump sits close to a fin\'s line and not far ahead of it, rather than staying quiet. It states the direction of that one term and no number. Clocking the shroud between the fins removes the question.',
      'A ROCKSIM AIRFOIL FIN SET NOW TAKES THE MASS THE FILE STATES. RockSim ignores the fin cross-section when it weighs a fin and OpenRocket applies a 0.85 factor, so the two disagree - and desktop OpenRocket resolves it by using the file\'s own number. Now so does this app. It is not one-directional: most files get about 18 percent heavier on the fin set, while newer RockSim files that model the taper themselves get much lighter - one two-stage design in the test library drops 679 grams, 18 percent of its dry weight, and its stability goes from 1.5 to 2.6 calibers. The number lands in Override mass and Override CG where you can see it and clear it.',
      'MOUNTING ANGLE AND DRAG: A CORRECTION TO SOMETHING THIS APP SAID WAS BROKEN. A camera shroud, a lug, a button and a drag bump are charged the same drag wherever they sit around the body, and that is CORRECT while the rocket flies straight - the air arrives the same way all round a round airframe, so turning a bump about the flight axis presents it with the same air and the same frontal area. Desktop OpenRocket and RASAero do the same. If it did scale with that angle, one click of this app\'s own "between fins" button would add 2.6 percent to a rocket\'s altitude. At an angle of attack there IS a real difference between the windward and leeward sides, and nobody models that.',
      'STILL NOT FIXED, and now with better reasons: motor thrust does not vary with ambient pressure; subsonic body drag runs high against the wind tunnel anchors, though a measured flight comparison says the error is being cancelled somewhere on the geometry most people fly, so there is no reference worth calibrating against yet; and the wake a shroud sheds onto a fin behind it is described but not modelled.',
    ],
  },
  {
    version: '0.102',
    date: '2026-09-04',
    title: 'Seven import defects fixed — RockSim and RASAero designs change shape, mass and balance, so re-open them',
    items: [
      'IF YOU HAVE IMPORTED A .rkt OR A .CDX1 FILE, RE-OPEN IT. Seven separate import defects are fixed in this release and most of them move mass, length, balance point or stability. The numbers you had were wrong; these ones match what the file itself says. Every change below was measured against the file\'s own recorded values, not against a guess.',
      'A ROCKSIM MASS OBJECT IS A POINT, AND WAS BEING SPREAD OVER ITS STATED LENGTH. RockSim stores a length for an avionics sled or a harness but treats the part as a point at the position you gave it - it does not even show that length in its own editor. The app was placing it at the middle of that length, so its balance point landed half a length away, and a length can be longer than the whole rocket. Measured: one two-stage design read a 7620 mm rocket for a 1652 mm airframe with nearly three times the true pitch inertia, and a 4 inch design read a static margin of 1.5 calibers where its own file says 4.2. The mass object now sits exactly where the file puts it, shown as a CG override you can clear.',
      'A NOSE CONE\'S BASE EXTENSION WAS DROPPED, SO THE ROCKET IMPORTED SHORT. RockSim cones can carry a cylindrical extension at the base; 53 of the 843 designs on hand have one, up to 127 mm. It was ignored, so the rocket came in shorter by that much and everything behind the cone sat too far forward. It is now a body tube of the same diameter directly behind the cone, named after it, and the parts aft of it land on the stations the file itself records. Desktop OpenRocket also drops this element, so these files now read LONGER here than they do there - that is deliberate, and the file\'s own station numbers are the evidence. Where the cone states a measured mass that mass already covered the extension, so the added tube carries none of its own.',
      'EVERY TUBE COUPLER AND ENGINE BLOCK WEIGHED EXACTLY NOTHING. Their wall thickness was being applied before the part was attached to its parent, and a part with an automatic outer diameter has no diameter yet at that moment - so the wall was clamped to zero and stayed there. This hit imported files AND every coupler or engine block ever added in the app, because those never carry an explicit diameter. A 4 inch fibreglass coupler was worth 0 g and is worth 27.4 g. Affected rockets get heavier, their balance moves forward, their margin rises slightly and their simulated altitude comes down a little. Centering rings with an automatic outer diameter were caught by the same fault.',
      'TUBE FIN WALL THICKNESS NEVER REACHED THE PHYSICS ENGINE. The box in the property panel was decoration: the tube fins were weighed and flown using the AIRFRAME\'S wall instead of their own. It moves the mass in either direction depending on which wall was thicker - measured, a set that should weigh 15.4 g read 7.9 g under a thin airframe and 29.5 g under a thick one - and it moves the centre of pressure too, because the tube\'s inner diameter drives its normal force.',
      'A ROCKSIM TRANSITION\'S SHAPE PARAMETER WAS READ FOR NOSE CONES AND NOT FOR TRANSITIONS. A power, Haack or parabolic transition was silently given the default curve instead of the one in the file. On one affected design that is 15 percent more drag at Mach 0.3 and half a caliber of stability. It is now read, and it is also written back on export, so a curve you edit here survives a trip to RockSim.',
      'ON RASAERO FILES, PARTS THAT OVERLAP WERE STACKED END TO END. A fin can slides over the tube in front of it and a boat tail slides into the booster below it; both were being added to the end of the rocket instead, making the design several percent too long. Measured on a two-stage file, 160.8 inches where the file\'s own position fields say 147.3 - 9.2 percent - with the centre of pressure 11.6 inches too far aft and the stability reading 5.59 calibers against a true 3.11. THIS CAN TURN A DESIGN THAT LOOKED STABLE INTO ONE THAT IS NOT: one bundled example goes from +0.59 calibers to -0.39, because the stacked fin can was hiding a real diameter step. Those are the honest numbers.',
      'AIRFOIL FIN SETS THIS APP WRITES INTO A .rkt ARRIVED IN DESKTOP OPENROCKET WEIGHING ZERO GRAMS. Desktop pins an airfoil fin set to RockSim\'s own computed mass, and we were not writing one - so it read the default of zero. Measured on a bundled test design, an 829 g fin set - 10.7 percent of the rocket\'s dry mass - vanished, and its stability was over-reported by 0.91 calibers. Every exported part now carries its computed mass and balance point.',
      'THE LAUNCH ROD EXIT SPEED WAS ALWAYS FLATTERING, BY 1.6 TO 4.3 PERCENT. It was read at the end of whichever calculation step carried the rocket past the end of the rod, by which point the rocket was already past it and still accelerating hard. It is now read at the exact instant the rocket leaves the guide, and the thrust-to-weight and the time off the rod come from that same instant instead of a slightly later one. On a short guide the old error reached 10 percent. A finer time step could never have fixed this - the engine floors its own step size before shortening it on the rod - so the number moves for everyone, and always downward. If you have been checking a marginal rod-exit speed against the 15 m/s guidance, check it again.',
      'THE DESCENT RATE IS NOW READ STRAIGHT FROM THE PHYSICS ENGINE. It was worked out from the altitude the rocket loses per second; the engine computes the vertical speed directly and used to throw it away before the app could see it. On a normal flight this moves nothing you can see - about a hundredth of a percent. It matters when the last parachute opens inside the last second or two before the ground, which used to read tens of percent fast, and it removes a case where a rocket that hit the ground at 46 ft/s with a chute still deploying could be reported as landing at zero.',
      'A NOTE ON MOUNTING ANGLE, WHICH WAS LISTED AS A DEFECT AND IS NOT ONE. A camera shroud, a launch lug, a rail button and a protuberance are charged the same drag wherever they sit around the body. That is correct while the rocket is flying straight: the air arrives the same way all round a round airframe, so turning a bump about the flight axis presents it with the same air and the same frontal area. Desktop OpenRocket and RASAero do the same. If drag did scale with that angle, one click of this app\'s own "between fins" button would add 2.6 percent to a rocket\'s altitude, which would be nonsense. What genuinely does depend on that angle - a shroud shedding its wake onto a fin behind it - is still not modelled, here or in any hobby package.',
    ],
  },
  {
    version: '0.101',
  ...
  },
  {
    ...
    items: [
      ...
      'OPENING A DESIGN NOW ASKS BEFORE DISCARDING UNSAVED WORK. There is ONE autosave slot, so opening a file replaced the working state and the next save overwrote the only copy of what was there - the work was genuinely gone. Opening now offers to save the current design first, and flying a simulation counts as a change worth keeping, as it does in desktop OpenRocket and RockSim. New also stops asking when there is nothing to lose, and the autosave is flushed when the tab closes rather than losing the last 400 ms of edits. A .rkt or .CDX1 export does NOT count as saved, because both formats drop things the .ork keeps.',
      'EVERY TUBE IN THE CATALOGUE IS REACHABLE IN THE SCALE DIALOG. The list collapsed every tube within 0.1 mm to a single entry with "(+N more)" after it, which hid 1,094 of the 1,309 body tubes - one entry alone stood for 87 of them, and a specific tube could not be selected at all. The sizes are now group headings and every part is an option under its size. Picking one also applies THAT tube’s own diameter rather than the first row in its group, which moves the resulting factor by up to 0.3 % against the part named on screen.',
      'EACH MOTOR MOUNT GETS ITS OWN SIZE in the Scale dialog: the scaled size, the nearest standard, any common casing size, or a number you type for an EX build. The nearest size is a recommendation, not an answer - a 4 inch LOC IV upscaled to a 7.51 inch tube wants a 75 mm mount by arithmetic and most people would build 98. The "snap to nearest" tick box stays as the default for everything you have not chosen.',
      'THE PARTS DATABASE IS TIDIER. Four companies were listed twice under slightly different spellings (SEMROC / SEMROC Astronautics, LOC Precision / LOC/Precision, BalsaMachining / BalsaMachining.com, Quest / Quest Aerospace), and MRC has been folded into MPC: 33 manufacturers become 29 (28 after the merges, plus a consistent AeroTech spelling to match the motor database). Nine rows that were the same part filed twice are gone and fourteen part numbers are corrected - including a pair of AeroTech couplers whose part number was the word "Aerotech", so they applied as "AeroTech Aerotech".',
      'A PART PULLED FROM THE DATABASE SHOWS ITS OWN MATERIAL instead of "Custom". The material and its density were always applied correctly - the flight was right - but the dropdown had no entry matching the catalogue’s name and fell back to Custom, which it did for 84 % of the database.',
      'Smaller: the Scale button no longer pushes Redo out of the Components panel on a narrow window, and a component whose part number differs only by a "+" (SEMROC BT-2+ against BT-2, a sleeve that slips over it) is no longer treated as a duplicate of it.',
    ],
  },
  {
    version: '0.090',
  ...
  },
  {
    version: '0.074',
    date: '2026-08-27',
    title: 'The Results tab stops throwing your flight away, and Save As finally asks where',
    items: [
      'YOUR CHARTS SURVIVE A CLICK. Opening a saved run from the table destroyed the plots for the flight you were looking at — deliberately, in one line — and there was no way back except pressing Launch again, which saves another row. That is where the duplicate rows in your run history came from. The shown flight now knows which run it belongs to, so clicking through the table and coming back restores its charts for free, and clicking the row you just flew does nothing at all instead of losing it.',
      'ANY OTHER SAVED RUN OF THE DESIGN ON SCREEN CAN GET ITS CHARTS BACK. A 📈 Charts button appears on rows the current design could still reproduce, and re-flies that exact flight — the physics is deterministic, so it is the same flight, not a new one — WITHOUT adding another row to the history. Clicking back and forth through history costs nothing after the first time. Where the design, motor or conditions have moved on since a run was flown, the button is absent and the app says why, rather than drawing a flight that is not the one in the table.',
      'SAVE AS ASKS WHERE TO PUT THE FILE — on Chrome and Edge, a real Save As dialog with the name prefilled and editable and the folder your choice. It never did before, on any browser: every save was the invisible download trick, so files landed in the Downloads folder under the rocket’s name with no dialog and nothing on screen. And because opening a file names the rocket after the file, saving reproduced that name exactly, which reads as “it overwrote my file”. On Firefox and Safari, which have no such dialog available to a web app, the download stays — but the app now names the exact file it wrote, so “I did a save and I don’t know where it went” cannot happen again. The 0.068 note claiming this button already asked every time was wrong; this is that promise being kept.',
      'THE AERODYNAMICS MODEL IS A SWITCH IN THE VITALS STRIP, on every workspace, instead of a read-only chip that appeared only when supersonic was on — so the model most people fly was never named anywhere. It changes the model FOR THIS SESSION ONLY and does not touch your saved preference: trying the other model on one design should not quietly become what every future session flies. Preferences remains the durable setting, says so while the strip is overriding it, and offers one click back; choosing a model there always wins and clears the override.',
      'CHANGING THE MODEL NO LONGER THROWS THE FLIGHT AWAY. It used to, so the strip could never show numbers from two different models at once — but that made comparing the two models impossible, which is the entire point of being able to switch them. The flight is kept and MARKED instead: the Results tab says which model those numbers were flown on, and the strip’s apogee carries a ⚠, until you press Launch again. Nothing is silently re-read under a model that did not produce it.',
      'EVERY DOWNLOAD BUTTON NAMES ITS DATA, with the format as the parenthetical. Three different files on the Results tab were all labelled “⬇ CSV”, and one of them wrote a 12-column subset under the name flight-data.csv — so the file that SOUNDED canonical was the poorest export on the page. That one is deleted: it was a lossless subset of the real flight-data CSV, minus the spreadsheet marker and minus every booster branch. The two real flight-data exports moved out of the launch report and up beside the plots they produce, under a Flight plots heading, and every export now stamps the design name into its filename.',
      'THE DESIGN CANVAS STOPPED PUSHING THE PAGE OFF THE BOTTOM OF THE SCREEN. Its height was a straight line in viewport height with a floor and no ceiling, and the chrome it budgeted for counted only what sits ABOVE it — so every extra pixel of window went to the canvas and the footer stayed exactly the same distance below the fold at every window size. It is clamped now, with the footer’s own height in the budget and a taller cap in ⟳90° mode, where height genuinely buys drawing rather than empty sky.',
      'THE NOTICE BAR STOPS SITTING ON THE FOOTER. It is fixed to the bottom of the window and nothing reserved space for it, so an active notice covered the footer’s links — the beta feedback path among them — and on a phone the tab bar painted over the notice itself. The bar now publishes its real height, the footer and the canvas both budget for it, and on a phone it sits above the tab bar rather than under it.',
      'A VERSION CHECK BESIDE THE VERSION BADGE: “✓ Up to date”, or “↻ v0.0NN available — Reload”. It reads what is actually deployed rather than anything cached, so a tab running an old build cannot be fooled by its own offline cache; offline it says “Version unknown” rather than guessing, and it will never claim you are out of date when it simply could not ask.',
      'TURNING THE FIRST-RUN TOUR OFF NOW WORKS, AND TURNING IT BACK ON DOES SOMETHING. The setting and the tour’s own “already seen” flag were two separate stored values, and the setting only ever wrote one of them — so Off looked inert and On could never re-show the tour to anyone who had seen it. The setting owns both now, and switching it off while the tour is on screen dismisses it, which is the literal thing a tester reported not working. The tour also counts itself seen when it is SHOWN rather than only when it is dismissed, so ignoring it and closing the tab no longer brings it back next visit.',
      'PREFERENCES THAT CANNOT BE SAVED SAY SO. Settings were written to browser storage and the failure swallowed, so in a private window or with site data blocked your choices worked for the session and silently reverted on reload — indistinguishable from a broken setting, and the other live explanation for the tour report above. Writes are verified now and a banner says what is happening, matching what autosave has always done.',
      'THE FLIGHT CONFIGURATIONS PANEL MOVED to the foot of Motors & Launch. It sat second on the tab and pushed the two panels a flyer actually works in below the fold on any file with several configurations. Apply now leads each row instead of hiding at the far right past a variable-length motor list, the list scrolls inside itself rather than growing without limit, and unloading every motor finally confirms that it did.',
      'THE CG AND CP MARKERS IN THE 3D VIEW ARE OPTIONAL — a ◉ CG/CP button on the view itself, and a four-way choice in Preferences, because the spheres on the axis and the floating callout beside the hull are separate drawings and wanting one without the other is a real request. Asked for by a tester who wanted to look at the shell.',
      'MASS ADDED TO AN OVERRIDDEN STAGE STOPS BEING A SILENT NO-OP — the first of the two limits shipped knowingly with 0.073. A stage override stands in for the mass of everything inside it, so a Build allowance placed there weighs nothing: you typed your scale reading, pressed Apply, a component appeared in the tree, and no number moved. Every RASAero .CDX1 import arrives with its stages pinned that way, so it was reachable by anyone importing one and then weighing their build. The box now names what is in the way and offers to pin THAT to your measured mass and CG instead — and only when a single override covers the whole rocket, because there is no rule for which of several stages should absorb the difference and a wrong override is worse than none. A related error goes with it: on exactly these designs the “Computed mass” line was reading light by the allowance’s own mass.',
      'A SAVED .ork NOW CARRIES THE FLIGHT RESULTS WE COMPUTED, so opening it in desktop OpenRocket shows your numbers instead of a table of blanks. Only for a configuration whose design, motors and conditions are unchanged since the run that produced them — change a fin and save again and that configuration goes back to “not simulated”, because a stale result sitting in desktop’s table looking exactly like a fresh one is the worst of the available outcomes. Only the summary travels; the per-timestep trace is not kept with run history, so desktop offers to re-run before it will plot.',
      'Internal: one place now hands a file to the browser, instead of ten copies of the same anchor trick — each of which revoked its blob on the very next line and could lose that race on a large workbook.',
    ],
  },
  {
    version: '0.073',
