...
describe('useDesignDirty — a restore that moved a pad mass onto the ranked primary', () => {
  // App's restore moves a weighed pad mass off a pod's record onto the core's
  // (padMassOntoRankedPrimary). The stored mark describes the design BEFORE
  // that move; the file on disk already is the moved design (seam review of
  // audit 2026-09-22 — App.session.test.tsx drives it through App).
  const stored = { mmt: { ...C6, padMassKg: 0.5 } };
  const moved = { mmt: C6 };

  it('re-takes the mark over the moved design when the stored mark described the design as stored', () => {
    const asStored = snap(tree('Saved'), stored);
    const h = mount(snap(tree('Saved'), moved), { savedMark: designFingerprint(asStored) },
      { motors: stored, configs: [] });
    expect(h.current.dirty).toBe(false);
    expect(h.current.savedMark.current).toBe(designFingerprint(snap(tree('Saved'), moved)));
  });

  it('leaves unsaved work its prompt: a mark that did not describe the design as stored stands', () => {
    const h = mount(snap(tree('Edited'), moved), { savedMark: designFingerprint(snap(tree('Saved'), stored)) },
      { motors: stored, configs: [] });
    expect(h.current.dirty).toBe(true);
  });
});
